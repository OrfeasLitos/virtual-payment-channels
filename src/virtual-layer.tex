...
\begin{center}
  ...
  \texttt{sig\_bal}, \texttt{left\_comms\_revkeys}) by $\bar{P}$:}
    \Comment{we are left intermediary}
    \begin{algorithmic}[1]
      ...
      \State output (\textsc{check coop close}, $(c_1, c_2)$,
      $\mathrm{sig}_{i-1, \mathrm{right}, C}$, $\pk{i-1, \mathrm{right}, R}$) to
      \texttt{guest}
      ...
      \State \trustedcode{$R_{\mathrm{loc}, \mathrm{right}} \gets$ TX \{input:
      $(c_{\mathrm{virt}}, 2/\{\pk{i, \mathrm{rev}}, \pk{i+1, \mathrm{rev}}\})$,
      output: $(c_{\mathrm{virt}}, \pk{i, \mathrm{out}})$\}}
      ...
      \mathrm{right}}$, $\mathrm{sig}_{i+1, \mathrm{right}, \mathrm{rev}}$,
      $\pk{i+1, \mathrm{rev}}$) = True}
      \State \trustedcode{output (\textsc{verify comm rev},
      \texttt{right\_comms\_revkeys}, \texttt{right\_revocations}) to
      \texttt{guest}}
      \State \trustedcode{extract $\mathrm{sig}_{\bar{P}, C, i+1}$,
      $\pk{\bar{P}, R, i+2}$ from \texttt{right\_comms\_revkeys}}
      ...
      \State \trustedcode{ensure \textsc{verify}($C_{P, i+1}$,
      $\mathrm{sig}_{\bar{P}, C, i+1}$, $\pk{\bar{P}, F}'$) = True}
      \State \trustedcode{extract $\mathrm{sig}_{\bar{P}, R, i}$ from
      ...
      \State $R_{\mathrm{loc}, \mathrm{left}} \gets$ TX \{input:
      $(c_{\mathrm{virt}}, 4/\{\pk{1, \mathrm{rev}}, \pk{i-1, \mathrm{rev}},
      \pk{i, \mathrm{rev}}, \pk{n, \mathrm{rev}}\})$, output:
      $(c_{\mathrm{virt}}, \pk{i, \mathrm{out}})$\} \Comment{the input
      corresponds to the revocation path of the virtual output of all virtual
      txs owned by $\bar{P}$}
      \State extract $\mathrm{sig}_{n, \mathrm{left}, \mathrm{rev}}$ from
      \texttt{right\_revocations}
      ...
      \State $R_{\mathrm{rem}, \mathrm{left}} \gets$ TX \{input:
      $(c_{\mathrm{virt}}, 2/\{\pk{i-1, \mathrm{rev}}, \pk{i,
      \mathrm{rev}}\})$, output: $(c_{\mathrm{virt}}, \pk{i-1, \mathrm{out}})$\}
      \State $\mathrm{sig}_{i, \mathrm{left}, \mathrm{rev}} \gets
      \textsc{sign}(R_{\mathrm{rem}, \mathrm{left}}, \sk{i, \mathrm{rev}})$
      \State send (\textsc{coop close back}, ((\texttt{right\_comms\_revkeys},
      $\mathrm{sig}_{i, \mathrm{left}, C}$, $\pk{i, \mathrm{left}, R}$),
      (\texttt{right\_revocations}, $\mathrm{sig}_{i, \mathrm{left},
      \mathrm{rev}}$, $\mathrm{sig}_{i, \mathrm{left}, R}$))) to $\bar{P}$ and
      expect reply (\textsc{coop close revocations}, \texttt{left\_revocations})
      \State extract $\mathrm{sig}_{i-1, \mathrm{right}, R}$,
      $\mathrm{sig}_{1, \mathrm{right}, \mathrm{rev}}$, $\mathrm{sig}_{i-1,
      ...
      $(c_{\mathrm{virt}}, 4/\{\pk{1, \mathrm{rev}}, \pk{i, \mathrm{rev}},
      \pk{i+1, \mathrm{rev}}, \pk{n, \mathrm{rev}}\})$, output:
      $(c_{\mathrm{virt}}, \pk{i+1, \mathrm{out}})$\}}
      \State \trustedcode{$\mathrm{sig}_{i, \mathrm{right}, \mathrm{rev}} \gets
      \textsc{sign}(R_{\mathrm{rem}, \mathrm{right}}, \sk{i, \mathrm{rev}})$}
      ...
      $\mathrm{sig}_{i, \mathrm{right}, \mathrm{rev}}$) to $\bar{P}$)}
    \end{algorithmic}
  \end{processbox}
  \captionof{figure}{}
