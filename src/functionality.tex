  ...
  \begin{systembox}{$\mathcal{F}_{\mathrm{Chan}}$ -- base}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \mathrm{out}}, F, \alice)$ to \adversary
      \EndIndent
      \Statex
  ...
  \begin{systembox}{$\mathcal{F}_{\mathrm{Chan}}$ -- corruption}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        F}$, $C =$ TX \{input: $F$.output, output: ($c_F, pk_{B,
        \mathrm{out}}$)\}) \Comment{Timelock redundant when \bob owns all coins}
        \State ensure $C$.verify($pk_{A, F}, pk_{B, F}$) = True
        \State $F \gets$ TX \{input: \texttt{base\_output}, output: $(c_F,
        2/\{pk_{A, F}, pk_{B, F}\})$\}
        \State $F \gets F\mathrm{.sign(}sk\mathrm{)}$
        \State $\mathit{State} \gets \textsc{waiting for ledger}$
