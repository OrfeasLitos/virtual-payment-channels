\section{Model \& Construction}
  ...
  \TODO{if this limitation is removed, change paragraph.} If the channel is virtual, each party also checks that its upcoming balance is
  ...
  \TODO{Add paragraph explaining steps for cooperative closure}

  When either party is instructed by \environment to unilaterally close the channel
...
On a related note, the
  ...
  When \textsc{virt} is instructed to unilaterally close by party $R$
  ...
  compatible virtual transaction. \TODO{The next lifts the balance limitation,
  so remove it if it isn't fixed after all. It however may be also relevant for
  the collaborative closing. Check this again} If any neighbour has spent the
  common funding output with an old, revoked state, \textsc{virt} creates a new
  virtual output (e.g.\ the one that would be produced by the malicious
  neighbour's intiator transaction) using the coins from the punishment of the
  revoked state. \textsc{virt} can then use its own virtual transaction as if
  its neighbour were honest. The \texttt{ANYPREVOUT} flag ensures that this
  newly generated output can be spent by \textsc{virt}'s virtual transaction.
