  ...
  \begin{systembox}{$\mathcal{F}_{\mathrm{Chan}}$ -- base}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        commitment TX in entry yet} \TODO{keys = $pk_{A, V}, pk_{B, V}$}
...
\TODO{restructure channel funding: on fund -> virt.fund(..., ln) which drives
ln.openVirutal(). Both in real and ideal (real does some referencing) and check
that virt - ln interactions are sane}
\begin{figure}[H]
  \begin{systembox}{$\mathcal{F}_{\mathrm{Chan}}$ -- virtual}
    \begin{algorithmic}[1]
      ...
      (\texttt{host\_funder}, \texttt{host\_fundee}), \texttt{keys}) by \alice:
      \TODO{\texttt{keys} = $pk_{\mathit{VA}, out}$, $pk_{\mathit{VB},
      out}$, use this in \textsc{virt.fund}() OR remove keys altogether}
      ...
      \Indent
        ...
        \State input (\textsc{open virtual}, $c$, \texttt{fundee},
        \texttt{host\_fundee}) to \texttt{fundee} as \alice, ensure output
        is (\textsc{ok})
