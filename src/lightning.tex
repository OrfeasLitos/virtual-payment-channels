    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \Else \: \Comment{real world}
          ...
          \textsc{host\_fundee}) to \texttt{fundee}
          \State \remotecode{ensure super.\textit{State} = \textsc{init}}
          \State \remotecode{store $pk_{A, F}$, $c$, \textsc{host\_fundee}}
          \State \remotecode{$(sk_{B, F}, pk_{B, F}) \gets \textsc{keyGen}()$}
      ...
      \texttt{host\_fundee}, $c$, \texttt{funding\_output}): \TODO{remove parts
      duplicated in \textsc{virtualKeys}}
      \Indent
        \State $(sk_{A, F}, pk_{A, F}) \gets \textsc{keyGen}()$
        \If{ideal world}
          \State $(sk_{B, F}, pk_{B, F}) \gets \textsc{keyGen}()$
