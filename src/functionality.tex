    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \Else \: \Comment{$\mathit{State} = \textsc{open virtual}$}
          \If{\alice is the one that had received (\textsc{fund you}, $\dots$)
          by \texttt{opener}}
            \State $\mathtt{initiator} \gets \mathtt{opener}$; $\mathtt{other}
            \gets \mathtt{outer\_peer}$
          \Else \: \Comment{\bob had received (\textsc{fund you} $\dots$)}
    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \Else
          ...
          had received (\textsc{fund you}, $\dots$), \texttt{outer\_peer} else)
          as \alice
        \EndIf
      \EndIndent
    \end{algorithmic}
