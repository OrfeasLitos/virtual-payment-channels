    ...
    \begin{algorithmic}[1]
      ...
      \Else \: \Comment{real world}
        ...
        \Else \: \Comment{opening virtual channel}
          \State \remotecode{$\textit{State}^B \gets \textsc{waiting for hosts
          ready}$}
        \EndIf
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln.prepareBase}()}
    \begin{algorithmic}[1]
      ...
      \Else \: \Comment{opening virtual channel}
        \If{ideal world}
          \State input (\textsc{host me}, $\bar{P}$, \texttt{hops}, $c$, $\pk{A,
          F}$, $\pk{B, F}$) to \texttt{hops}[0].\texttt{left} and expect output
          (\textsc{hosts ready}, \texttt{host\_fundee}) to \texttt{fundee} by
          \texttt{hops}[-1].\texttt{right} \Comment{ignore any other message}
          ...
          \texttt{hops}[0].\texttt{left} \TODO{make this work with
          \textsc{virt}} \TODO{use \texttt{left} similarly in \textsc{virt}}
        \Else \: \Comment{real world}
          \State input (\textsc{host me}, \alice, \bob, \texttt{hops}, $c$,
          $\pk{A, F}$, $\pk{B, F}$) to \texttt{hops}[0].\texttt{left} and expect
          output (\textsc{hosts ready}, \texttt{host\_funder}) \Comment{ignore
          any other message}
        \EndIf
      \EndIf
    \end{algorithmic}
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln.exchangeOpenSigs}()}
    \begin{algorithmic}[1]
      ...
      (\textsc{hosts ready}, \texttt{host\_fundee}) by
      \texttt{hops}[-1].\texttt{right} (c.f. Fig~\ref{code:ln:bob},
      line~\ref{code:ln:bob:host})} \TODO{do this exchange in last hop,
      \textsc{virt}}
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- external open messages for \bob}
    \begin{algorithmic}[1]
      ...
      \State On output (\textsc{hosts ready}, \texttt{host\_fundee}) by
      \texttt{hops}[-1].\texttt{right}: \Comment{real world}
      \Indent
        \State ensure $\textit{State}^B = \textsc{waiting for hosts ready}$
        \State store \texttt{host\_fundee} \Comment{we will talk directly to
        \texttt{host\_fundee}}
  ...
  \begin{processbox}{\textsc{ln} -- virtualise}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \texttt{hops}, \texttt{fundee}, $c$) to $\bar{P}$ and expect reply
        (\textsc{virtualising ack}, $\texttt{host}_{\bar{P}}$, $\pk{\bar{P},
        F}'$)
        ...
        $\texttt{host}_{\bar{P}}$, \TODO{old hops}, $c$, $\pk{A, V}$, $\pk{B,
        V}$, $\pk{P, F}'$, $\pk{\bar{P}, F}'$) to $\texttt{host}_P$ and expect
        reply (\textsc{hosts ready})
        \State output (\textsc{hosts ready}, $\texttt{host}_P$) to
        \texttt{funder}
        \State \TODO{change \texttt{hops} to \texttt{host\_funder} (notice the
        bad naming convention)}
      \EndIndent
      \Statex

      \State On (\textsc{virtualising}, $\texttt{host}_{\bar{P}}$, $\pk{\bar{P},
      F}'$, \texttt{hops}, \texttt{fundee}, $c$) by $\bar{P}$:
      \Indent
        \State ensure $\textit{State}^P = \textsc{open}$
        ...
        transfer $c$ coins instead of locking them, but the end result is the
        same}
        \State $(\sk{P, F}', \pk{P, F}') \gets \textsc{keyGen}()$
        \If{$\mathrm{len}(\texttt{hops}) > 1$} \Comment{we are not the last hop}
          \State define new \textsc{virt} ITI $\texttt{host}_P$
          \State input (\textsc{virtualising}, $\texttt{host}_P$, ($\sk{P, F}'$,
          $\pk{P, F}'$), $\pk{\bar{P}, F}'$, \texttt{hops}[1:], \texttt{fundee},
          $c$) to \texttt{hops}[1].\texttt{left} and expect reply
          (\textsc{virtualising ack}, \texttt{host\_sibling}, $\pk{\mathrm{sib},
          \bar{P}, F}$)
          \State input (\textsc{init}, $\texttt{host}_{\bar{P}}$,
          \texttt{host\_sibling}, $\pk{P, F}'$, $\pk{\bar{P}, F}'$,
          $\pk{\mathrm{sib}, \bar{P}, F}$, $c$) to $\texttt{host}_P$ and expect
          reply (\textsc{host init ok})
        \Else \: \Comment{we are the last hop}
          \State input (\textsc{init}, $\texttt{host}_{\bar{P}}$, $\pk{P, F}'$,
          $\pk{\bar{P}, F}'$, $c$) to new \textsc{virt} ITI $\texttt{host}_P$
          and expect reply (\textsc{host init ok})
        \EndIf
      ...
      \texttt{fundee}, $c$) by \texttt{sibling}:
      \Indent
        \State ensure $\textit{State}^P = \textsc{open}$
        \State ensure $c_P - \texttt{locked}_P \geq c$
        \State $\textit{State}^P \gets \textsc{virtualising}$
        ...
        \State define new \textsc{virt} ITI $\texttt{host}_P$
        \State send (\textsc{virtualising}, $\texttt{host}_P$, $\pk{P, F}'$,
        \texttt{hops}, \texttt{fundee}, $c$) to \texttt{hops}[0].\texttt{right}
        and expect reply (\textsc{virtualising ack}, $\texttt{host}_{\bar{P}}$,
        $\pk{\bar{P}, F}'$)
        \State ensure $\pk{\bar{P}, F}'$ is different from $\pk{\bar{P}, F}$ and
        all older $\bar{P}$'s funding public keys
        \State input (\textsc{init}, $\texttt{host}_{\bar{P}}$,
      ...
      \Comment{real or ideal world}
      \Indent
        \State ensure $\textit{State}^P = \textsc{waiting for hosts ready}$
        \State $\textit{State}^P \gets \textsc{waiting for comm sig}$
        \TODO{update former base channel as well}
    ...
    \begin{algorithmic}[1]
      ...
      \Else \: \Comment{counterparty is idle}
        ...
        can spend} \Comment{possibly due to an active timelock}
          ...
          $\Sigma$
        \EndWhile
        \State \Comment{provably reachable -- c.f.~\TODO{ref}}
