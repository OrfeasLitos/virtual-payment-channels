...
Interconnecting state channels in LN enables any two parties to transmit funds
...
its usefulness, it can be closed off-chain if involved parties cooperate.
...
become an initiator towards closing the channel, while subsequent respondents, following the activation sequence, can choose the right action to successfully complete the closure process by posting a single transaction each.
  ...
  Universally Composable security (UC)~\cite{uc} setting; our ideal functionality is global, according to the
  definition of~\cite{DBLP:conf/tcc/BadertscherCHTZ20}. The construction relies
  on the \texttt{ANYPREVOUT} signature type (currently under discussion for
  future inclusion in Bitcoin), which does not sign the hash of the transaction it spends,
