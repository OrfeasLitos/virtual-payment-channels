  ...
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        $(\pk{A, V}, \pk{B, V})$)
        \State ensure \textsc{virt.circulateHostData}(\texttt{hops}) returns
        (\textsc{ok})
      ...
      \State \textsc{circulateHostData}(\texttt{hops}, $\pk{\mathrm{left}}$):
      \Indent
        \State \Comment{\texttt{hops} is a list of \textit{pid} pairs}
        \State $(\sk{\mathrm{loc}}, \pk{\mathrm{loc}}) \gets
        \textsc{keyGen}()$
        \If{$\pk{\mathrm{left}}$ is given as argument} \Comment{we are not
        \texttt{host\_funder}}
          \If{len(\texttt{hops}[1:]) $\geq 1$} \Comment{we are not
          \texttt{host\_fundee}}
            \State input (\textsc{hostDataForward}, $\texttt{hops}$[1:],
            $\pk{\mathrm{left}}$, $(\sk{\mathrm{loc}}, \pk{\mathrm{loc}})$) to
            \texttt{hops}[1].\alice{} \Comment{sibling}
            \State \trustedcode{store $\pk{\mathrm{left}},
            (\sk{\mathrm{loc}}, \pk{\mathrm{loc}})$}
            \State \trustedcode{call
            \textsc{virt.circulateHostData}(\texttt{hops}, $\pk{\mathrm{loc}}$)
            of \texttt{hops}[0].\bob and assign output to $\pk{\mathrm{right}}$}
            \State \trustedcode{$(\texttt{comm\_keys\_loc},
            \texttt{comm\_keys\_rem}) \gets \textsc{pcn.getCommKeys}()$}
            \State \trustedcode{output (\textsc{HostDataBack},
            $\pk{\mathrm{right}}$, $c_{\alice}$, $c_{\bob}$,
        ...
        \Else \: \Comment{we are \texttt{host\_funder}}
          \State call \textsc{virt.circulateHostData}(\texttt{hops},
          $\pk{\mathrm{loc}}$) of \texttt{hops}[0].\bob and assign output to
          $\pk{\mathrm{right}}$
          \State \Return (\textsc{ok})
        \EndIf
      \EndIndent
