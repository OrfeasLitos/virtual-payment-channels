...
\begin{center}
  ...
  $\texttt{sig\_bal}_{\bar{P}}$) by $\bar{P}$:}
    \begin{algorithmic}[1]
      ...
      \State \trustedcode{$R_{\mathrm{loc}} \gets$ TX \{input:
      ...
      \mathrm{rev}}$ from \texttt{right\_revocations}}
      \State \trustedcode{ensure \textsc{verify}($R_{\mathrm{loc},
      \mathrm{right}}$, $\mathrm{sig}_{2, \mathrm{right}, \mathrm{rev}}$,
      $\pk{2, \mathrm{rev}}$) = True}
      ...
      \State \trustedcode{$R_{\mathrm{rem}, \mathrm{right}} \gets$ TX \{input:
      ...
      \gets \textsc{sign}(R_{\mathrm{rem}}, \sk{1,
      \mathrm{rev}})$}
      ...
      \mathrm{rev}}$, $(\mathrm{sig}_{1, j, \mathrm{left}, \mathrm{rev}})_{j \in
      \{2, \dots, n\}}$)) to $\bar{P}$}
    \end{algorithmic}
  \end{processbox}
  \captionof{figure}{}
