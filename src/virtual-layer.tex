\begin{figure}[H]
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      ($\sk{P, F}$, $\pk{P, F}$), $\pk{\mathrm{rem}, \mathrm{virt}}$) by
      \texttt{guest}:
      ...
      \Indent
        ...
        \texttt{host\_funder}}
          ...
          \texttt{host\_fundee}}
            ...
            $(\sk{\mathrm{loc}, \mathrm{virt}}, \pk{\mathrm{loc},
            \mathrm{virt}})$, \texttt{left\_fund\_keys}}
            \State \trustedcode{call
            \textsc{virt.circulateVirtualKeys}(\texttt{hops}, ($\pk{A, V}$,
            $\pk{B, V}$), ($\pk{\mathrm{loc}, \mathrm{virt}}$,
            \texttt{left\_virt\_keys}[0])) of \texttt{hops}[0].\bob and assign
            output to \texttt{right\_virt\_keys}, \texttt{right\_fund\_keys}}
