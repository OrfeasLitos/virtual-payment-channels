    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        \If{$\mathit{State} = \textsc{open base}$}
          ...
          \EndIf \Comment{note that rest is run only if \alice is honest}
        ...
        \ElsIf{$\mathit{State} = \textsc{open virtual}$} \TODO{think more about
        closing w/ one corrupted party}
          \State $\mathit{State} \gets \textsc{closed}$
      ...
      \Indent
        \State ensure that exactly one party in $\{\alice, \bob\}$ is corrupted
        \State wlog, let \alice corrupted, \bob honest
...
\begin{figure}[H]
  \begin{systembox}{$\mathcal{F}_{\mathrm{Chan}}$ -- corruption}
    \begin{algorithmic}[1]
      ...
      \TODO{receive payment from corrupted counterparty}
    \end{algorithmic}
  \end{systembox}
  \caption{}
