\section{Efficiency Evaluation \& Simulations}
  ...
  % splncs
  %\addotlength{\intextsep}{-25pt}
  \begin{table*}[h!]
    \begin{minipage}{\textwidth}
    \centering
    \begin{tabular}{|l|c|c|c|c|c|c|}
    \hline
    \multicolumn{7}{|c|}{Delays (sec) and Relative Fees} \\
    \hline
        & \multicolumn{2}{|c|}{Power Law}
        & \multicolumn{2}{|c|}{Preferred Receiver}
        & \multicolumn{2}{|c|}{Uniform} \\
    \hline
        & Delay (sec) & Fees & Delay (sec) & Fees & Delay (sec) & Fees \\
    \hline
    LVPC & {\color{red}$2873$} & {\color{blue}$0.81$} & {\color{red}$2440$} & {\color{blue}$0.80$} & {\color{blue}$3589$} & {\color{blue}$0.99$} \\
    \hline
    Donner & {\color{blue}$2842$} & {\color{red}$1$} & $2361$ & {\color{red}$1$} & {\color{blue}$3589$} & {\color{red}$1$} \\
    \hline
    Elmo & $2864$ & $0.83$ & {\color{blue}$2344$} & $.87$ & {\color{blue}$3589$} & {\color{blue}$0.99$} \\
    \hline
    \end{tabular}
    \end{minipage}
      \caption{Average per-payment delay (both on- and off-chain) and relative
      fee. Less is better. {\color{blue}Blue} is best, {\color{red}red} is
      worst.}
  ...
  We offer a cost and efficiency comparison of this work with
  LVPC~\cite{10.1007/978-3-030-65411-5_18} and Donner~\cite{donner}. We focus on
  these due to their exclusive support of
  virtual channels over any number of base channels. We remind the reader that LVPC
  achieves this via recursion and Donner via
  its variadic property (cf. Table~\ref{table:comparison-features}).
  ...
  % IEEEtrans
  Comparison details and choices are deferred to~\cite{full-version}.
...
%
  ...
  % IEEEtrans
  framework\footnote{\texttt{gitlab.com/anonymised-submission-8778e084/}\\\texttt{virtual-channels-simulation}}
  ...
  % acmart
%  The knowledge function defines which future payments inform each decision.
...
%
  ...
  channels aim to improve, namely payment latency and
  fees (Table~\ref{table:performance}). We have designed 3 workloads: ``power law'', in
  ...
  % IEEEtrans
  More details are deferred to~\cite{full-version}.
...
%
  ...
  %fees are not systematically chosen, therefore Table~\ref{table:performance} provides
  %relative, not absolute, fees.
%
  %As Table~\ref{table:performance} shows, delays are primarily influenced by the
