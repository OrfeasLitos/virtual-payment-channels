    ...
    \begin{algorithmic}[1]
      \Indent
        ...
        $\mathrm{TX}_{\mathrm{loc}, \mathrm{both}}$, $C$)}
          ...
          $\mathrm{sigs}_{\mathrm{byLeft}}[i]$, \texttt{left\_virt\_keys}[0]) =
          True
        \EndFor
    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \Comment{we are not \texttt{host\_funder}}
          ...
          \If{len(\texttt{hops}[1:]) $\geq 1$} \Comment{we are not
          \texttt{host\_fundee}}
            \State input (\textsc{virtualBaseSigForward}, \texttt{hops}[1:],
            ...
            $\textsc{sign}(\mathrm{TX}_{\mathrm{next}, \mathrm{none}}$) of
            \texttt{hops}[0].\bob and assign output to
            $\mathrm{sig}_{\mathrm{loc}, \mathrm{none}}$}
            \State \trustedcode{ensure \textsc{verify}($\mathrm{TX}_{\mathrm{loc},
            \mathrm{none}}$, $\mathrm{sig}_{\mathrm{loc}, \mathrm{none}}$,
            \texttt{right\_virt\_keys}[0]) = True}
            \State \trustedcode{output (\textsc{virtualBaseSigBack},
