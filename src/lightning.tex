    ...
    \begin{algorithmic}[1]
      ...
      \Else \: \Comment{opening virtual channel}
        \If{ideal world}
          ...
          \texttt{fundee} by \texttt{hops}[-1].\texttt{right} \Comment{ignore
          any other message}
  ...
  \begin{processbox}{\textsc{ln} -- virtualise}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \State \TODO{also give our previous \textsc{hosts} to \textsc{virt}}
      \EndIndent
      \Statex
      ...
      \Indent
        ...
        \If{$\mathrm{len}(\texttt{hops}) > 1$} \Comment{we are not the last hop}
          ...
          $\pk{P, F}'$), \texttt{hops}[1:], \texttt{fundee}, $c$) to
          \texttt{hops}[1].\texttt{left} and expect reply (\textsc{virtualising
          ack}, \texttt{host\_sibling}, $\pk{\mathrm{sib}, \bar{P}}$)
      ...
      F}'$, $\pk{P, F}'$), \texttt{hops}, \texttt{fundee}, $c$) by
      \texttt{sibling}:
      \Indent
        \State ensure $\textit{State}^P = \textsc{open}$
        \State ensure $c_P - \texttt{locked}_P \geq c$
        ...
        all older $\bar{P}$'s funding public keys
        \State input (\textsc{init}, $\texttt{host}_{\bar{P}}$,
        \texttt{host\_sibling}, $\pk{P, F}'$, $\pk{\bar{P}, F}'$, $c$) to
