  ...
  \begin{processbox}{\textsc{ln} -- virtualise}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        $\pk{\bar{P}, F}$, $\pk{\bar{P}, F}'$) to $\texttt{host}_P'$ and expect
      ...
      \Indent
        ...
        \If{$\mathrm{len}(\texttt{hops}) > 1$} \Comment{we are not the last hop}
          ...
          $c$, $c_{\bar{P}}$, $c_P$) to \texttt{hops}[1].\texttt{left} and
        ...
        \Else \: \Comment{we are the last hop}
          ...
          $c$) to new \textsc{virt} ITI $\texttt{host}_P'$ and expect reply
          (\textsc{host init ok})
        \EndIf
      ...
      \texttt{fundee}, $c$, $c_{\mathrm{sib}, \mathrm{rem}}$, $_{\mathrm{sib}}$)
      by \texttt{sibling}:
      \Indent
        \State ensure $\textit{State}^P = \textsc{open}$
        \State ensure $c_P - \texttt{locked}_P \geq c$
        ...
        attack: one counterparty closes with old version, the other does not
        close}
        \State $\textit{State}^P \gets \textsc{virtualising}$
