\section{Protocol Description}
  ...
  In more detail, during the channel opening procedure (in the full version) the two counterparties (i) create new keypairs and
  ...
  of the protocol to aid comprehension --- see the full version for
  more details.
  ...
  \begin{figure}[!htbp]
    ...
    $A_F$ and $A_R$ are $A$'s funding and revocation keys respectively. $A+t$ needs a signature
    by $A$ after relative timelock of $t$. The first $C_A$ output is
  ...
  signatures ($2$ messages). $A$ then publishes the $F$ on-chain. Once it
  ...
  commitment txs. $3$ messages are needed for a payment. $B$'s revocation tx ($R_B$ in
  ...
  Bridge txs are discussed in the full version.
  ...
  What if one party does not cooperate? Then some of the other parties must close
