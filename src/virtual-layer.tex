    ...
    \begin{algorithmic}[1]
      ...
      \texttt{host\_funder}}
        ...
        \texttt{host\_fundee}}
          ...
          $\pk{\mathrm{rem}, F}$, $\pk{\mathrm{loc}, \mathrm{out}}$, $c_P$,
          $c_{\bar{P}}$) to \texttt{sibling}
          ...
          $\pk{\mathrm{sib}, \mathrm{out}}$, $c_{\mathrm{sib}}$,
          $c_{\mathrm{sib}, \mathrm{rem}}$} \Comment{remove parentheses as
          necessary}
          ...
          \mathrm{virt}}$, $\pk{\mathrm{loc}, \mathrm{out}}$)
...
\begin{processbox}{\textsc{virt}}
  \begin{algorithmic}[1]
    ...
    \State \textsc{getMidTXs}($i$, $n$, $c_{\mathrm{virt}}$,
    $c_{\mathrm{loc}}$, $c_{\mathrm{rem}}$, $c_{\mathrm{sib}}$,
    $c_{\mathrm{sibRem}}$, $\pk{\mathrm{left}, \mathrm{fund}, \mathrm{old}}$,
    $\pk{\mathrm{loc}, \mathrm{fund}, \mathrm{old}}$, $\pk{\mathrm{sib},
    \mathrm{fund}, \mathrm{old}}$, $\pk{\mathrm{right}, \mathrm{fund},
    \mathrm{old}}$, $\pk{\mathrm{left}, \mathrm{fund}, \mathrm{new}}$,
    $\pk{\mathrm{loc}, \mathrm{fund}, \mathrm{new}}$, $\pk{\mathrm{sib},
    \mathrm{fund}, \mathrm{new}}$, $\pk{\mathrm{right}, \mathrm{fund},
    \mathrm{new}}$, $\pk{\mathrm{left}, \mathrm{virt}}$, $\pk{\mathrm{right},
    \mathrm{virt}}$, $\{\pk{i,j,k}\}_{i \in [n], j \in [n-1] \setminus \{1\}, k
    \in [n-1] \setminus \{1, j\}}$, $\{\pk{i, 2, 1}\}_{i \in [n]}$,
    $\{\pk{i, n-1, n}\}_{i \in [n]}$, $(t_i)_{i \in [n-1] \setminus \{1\}}$):
    \Indent
      \State ensure $1 < i < n$
      \State ensure $c_{\mathrm{sibRem}} \geq c_{\mathrm{virt}} \wedge
      c_{\mathrm{loc}} \geq c_{\mathrm{virt}}$ \Comment{left parties fund
      virtual channel}
...
\begin{processbox}{\textsc{virt}}
  \begin{algorithmic}[1]
    ...
    \State \textsc{getEdgeTXs}($i$, $n$, $c_{\mathrm{virt}}$,
    ...
    \Indent
      ...
      \EndIf
      \State $\mathrm{TX}_1 \gets \mathrm{TX}$: \Comment{edge nodes have only
      a first-mover tx}
      \Indent
        \State inputs:
        \Indent
...
\begin{figure}[H]
  ...
  \{\texttt{guest}, \texttt{funder}, \texttt{fundee}\}$:}
    \begin{algorithmic}[1]
      ...
      \Comment{we are an edge node and our counterparty has closed}
