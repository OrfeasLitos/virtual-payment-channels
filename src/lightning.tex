    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \Else \: \Comment{real world}
          ...
          implicit}
          \State \foreigncode{ensure super.\textit{State} = \textsc{init}}
          \Comment{``super'': storage of enclosing protocol}
          \State \foreigncode{store $pk_{A, F}$, $pk_{A, \mathrm{out}}$}
          \State \foreigncode{$(sk_{B, F}, pk_{B, F}) \gets \textsc{keyGen}()$}
          \State \foreigncode{reply (\textsc{accept base channel}, $pk_{B,
          F}$, $pk_{B, \mathrm{out}}$)}
        \EndIf
        ...
        \If{real world}
          ...
          previous exchange}
          \State \foreigncode{$F \gets$ TX \{input: \texttt{base\_output}, output:
          $(c_{\mathrm{chain}}, 2/\{pk_{A, F}, pk_{B, F}\})$\}}
          \State \foreigncode{$C_{B, 0} \gets$ TX \{input: $F$.output, outputs:
          ($c_{\mathrm{chain}}$, $pk_{A, \mathrm{out}}$), ($0$, $pk_{A,
          \mathrm{out}}$ $\wedge$ $pk_{B, \mathrm{out}}$ $\wedge$
          $\texttt{delay}$)\}}
          \State \foreigncode{ensure \textsc{verify}($C_{B, 0}$,
          $\mathrm{sig}_{A, C, 0}$, $pk_{A, F}$) = True}
          \State \foreigncode{$C_{A, 0} \gets$ TX \{input: $F$.output, outputs:
          ($c_{\mathrm{chain}}$, $pk_{A, \mathrm{out}}$ $\wedge$ $pk_{B,
          \mathrm{out}}$ $\wedge$ $\texttt{delay}$), ($0$, $pk_{B,
          \mathrm{out}}$)\}}
          \State \foreigncode{$\mathrm{sig}_{B, C, 0} \gets \textsc{sign}(C_{A,
          0}, sk_{B, F})$}
          \State \foreigncode{reply (\textsc{funding signed}, $\mathrm{sig}_{B,
          C, 0}$)}
    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \Else \: \Comment{real world}
          ...
          \State \foreigncode{store $pk_{A, F}$, $pk_{A, \mathrm{out}}$}
          \State \foreigncode{$(sk_{B, F}, pk_{B, F}) \gets \textsc{keyGen}()$}
          \State \foreigncode{reply (\textsc{accept virtual channel}, $pk_{B,
          F}$, $pk_{B, \mathrm{out}}$)}
        \EndIf
        \State send (\textsc{funding keys}, $c$, $pk_{A, F}$, $pk_{B, F}$) to
        \texttt{host\_funder}
        \State \foreigncode{$C_{\mathrm{temp}} \gets C_i$ with $c$ coins moved
        from our output to new 2/\{$pk_{A, F}$, $pk_{B, F}$\} output named
        \texttt{funding\_output}} \TODO{make more formal}
        \State \foreigncode{reply (\textsc{funding output},
