...
\begin{center}
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \mathrm{out}}$, $(\pk{h, \mathrm{rev}})_{h \in [n]}$, $(\pk{h,j,k})_{h \in
      ...
      \Indent
        ...
        \ForAll{$j \in [n-1] \setminus \{1\}, k \in [n-1] \setminus \{1, j\}$}
          ...
          k}\} \wedge \texttt{"}k\texttt{"}) \vee n/\{\pk{1, \mathrm{rev}},
          \dots, \pk{n, \mathrm{rev}}\}$
        \EndFor
        \If{$i = 2$}
          \State $\mathit{all}_{2, 1} \gets (n/\{\pk{1, 2, 1}, \dots, \pk{n, 2,
          1}\} \wedge \texttt{"1"}) \vee n/\{\pk{1, \mathrm{rev}}, \dots,
          \pk{n, \mathrm{rev}}\}$
        \EndIf
        \If{$i = n-1$}
          \State $\mathit{all}_{n-1, n} \gets (n/\{\pk{1, n-1, n}, \dots, \pk{n,
          n-1, n}\} \wedge \texttt{"}n\texttt{"}) \vee n/\{\pk{1,
          \mathrm{rev}}, \dots, \pk{n, \mathrm{rev}}\}$
        \EndIf
...
\begin{center}
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \mathrm{virt}}$, $\pk{\mathrm{right}, \mathrm{virt}}$, $(\pk{h,
      \mathrm{rev}})_{h \in [n]}$, $(\pk{\mathrm{all}, j})_{j \in [n]}$, $t$):
      \Indent
        ...
        \If{$i = 1$} \Comment{funder's tx}
          ...
          \pk{\mathrm{all}, n}\} \wedge \texttt{"1"}) \vee n/\{\pk{1,
          \mathrm{rev}}, \dots, \pk{n, \mathrm{rev}}\}$
        \Else \: \Comment{fundee's tx}
          \State $\mathit{all} \gets (n/\{\pk{\mathrm{all}, 1}, \dots,
          \pk{\mathrm{all}, n}\} \wedge \texttt{"}n\texttt{"}) \vee n/\{\pk{1,
          \mathrm{rev}}, \dots, \pk{n, \mathrm{rev}}\}$
        \EndIf
...
\begin{center}
  \begin{processbox}{\textsc{virt} -- On input (\textsc{coop close},
  \texttt{sig\_bal}, \texttt{comms\_revkeys}) by $R$:}
    \begin{algorithmic}[1]
      \State ensure $\itistate = \textsc{open}$ \TODO{decide if we should inform
      $R$ when we are in some other state}
      \If{\texttt{funder} is defined} \Comment{we are endpoint}
        \State ensure $R = \texttt{funder}$
        \State output our input to \texttt{guest} and expect reply (\textsc{coop
        close back}, \texttt{comms\_revkeys\_back}, \texttt{virt\_revocations})
        \State $\mathrm{TX}_{\mathrm{punish}, \mathrm{virt}} \gets$ TX \{input:
        $\mathrm{TX}_{1, 1}$.outputs[0], output: $\pk{1, \mathrm{out}}$\}
        \Comment{spends output carrying $c_{\mathrm{virt}}$}
        \State $\mathrm{TX}_{\mathrm{punish}, \mathrm{fund}} \gets$ TX
        \{input: $\mathrm{TX}_{1, 1}$.outputs[0], output: $\pk{1,
        \mathrm{out}}$\} \Comment{spends other output}
        \State parse \texttt{virt\_revocations} as
        $(\mathrm{sig}(\mathrm{TX}_{\mathrm{punish}, \mathrm{virt}}, \pk{h,
        \mathrm{rev}}))_{h \in [n] \setminus \{1\}}$,
        sig($\mathrm{TX}_{\mathrm{punish}, \mathrm{fund}}, \pk{2, \mathrm{rev}}$)
        \State ensure signatures are valid
        \State output (\textsc{coop close back}, \texttt{comms\_revkeys\_back})
        to \texttt{funder}
        \State output our input to \texttt{guest} and expect reply
      \Else \: \Comment{we are intermediary}
        \State ensure $R = \texttt{guest}$ \Comment{may be redundant due to UC
        constraints}
        \State input our input to \texttt{sibling} and expect reply
      \EndIf
    \end{algorithmic}
  \end{processbox}
  \captionof{figure}{}
  \label{code:virtual-layer:coop-close}
\end{center} \ \\
...
\begin{center}
  ...
  c_2)$) by \texttt{fundee}:}
    \begin{algorithmic}[1]
      ...
      \State extract $\mathrm{sig}_{i, \mathrm{right}, C}$, $\pk{i, R}$ from
      \texttt{left\_comms\_revkeys}
      \State output (\textsc{check coop close}, $(c_1, c_2)$, $\mathrm{sig}_{i,
      \mathrm{right}, C}$, $\pk{i, R}$) to \texttt{guest}
      ...
      $(c_P + c_1, \pk{P, \mathrm{out}})$, $(c_P + c_2, (\pk{\bar{P},
