...
\begin{center}
  \begin{processbox}{\textsc{ln.exchangeOpenSigs}()}
    \begin{algorithmic}[1]
      ...
      \mathrm{out}} + (p + s)$) $\vee$ $2/\{\pk{A, R}, \pk{B, R}\}$)\}}
...
\begin{center}
  \begin{processbox}{\textsc{ln}.\textsc{revocationsTrip}()}
    \begin{algorithmic}[1]
      ...
      x$; $i \gets i + 1$}
      \State \remotecode{$\itistate \gets \textsc{open}$}
      \If{\remotecode{$\texttt{host}_P \neq$ ``\texttt{ledger}'' $\wedge \bar{P}$ has a
      \texttt{host\_sibling}}} \Comment{we are intermediary channel}
...
\begin{center}
  \begin{processbox}{\textsc{ln} -- On (\textsc{pay}, $x$):}
    \begin{algorithmic}[1]
      ...
      \If{$\texttt{host}_P \neq$ ``\texttt{ledger}'' $\wedge P$ has a \texttt{host\_sibling}}
  ...
  \begin{processbox}{\textsc{ln} -- On (\textsc{coopClose}):}
    ...
    \begin{algorithmic}[1]
      ...
      \If{we are not \texttt{fundee}}
        \State send (\textsc{coop close balance}) to \texttt{fundee} and expect
        output (\textsc{coopClosed}) by $\texttt{host}_P$
...
\begin{center}
  \begin{processbox}{\textsc{ln} -- On (\textsc{coop close balance}) by $R$}
