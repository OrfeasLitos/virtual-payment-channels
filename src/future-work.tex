...
\@ifclassloaded{IEEEtran}%
  ...
  {\paragraph{Future work.}}%
\makeatother%
  Further usability enhancements are possible: Firstly, it is possible to
  support multiple virtual channels in the same virtual layer, reducing on-chain
  and timelock requirements. Secondly, the maximum time between
  activations can be turned from the currently global constant into a
  per-channel configurable parameter. Thirdly, various non-malicious mishaps
  ...
  the latter.
  For more details on future work see Appx.~\ref{sec:further-future-work}.
