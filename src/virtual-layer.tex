\begin{figure}[H]
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      \State On input (\textsc{init}, $\texttt{host}_{\bar{P}}$,
      \texttt{sibling}, $\pk{P, F}$, $\pk{\bar{P}, F}$, $\pk{\mathrm{sib},
      \bar{P}, F}$, $c$) by \texttt{guest}:
      \Indent
      ...
      $\bar{P}$, \texttt{hops}, $c$, $\pk{A, V}$, $\pk{B, V}$, $\pk{A, F}$,
