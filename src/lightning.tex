\TODO{if floating channel, setup on \textsc{open} and use on \textsc{close} the
intermediary -AZall->O-AZ-> TX}
\begin{figure}[H]
  ...
  \begin{processbox}{\textsc{ln} -- methods used by \textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \sk{P, R})$
        \If{$\textit{State}^P = \textsc{waiting for revocation}$}
          \State $\textit{State}^P \gets \textsc{waiting for inbound revocation}$
        \Else \: \Comment{$\textit{State}^P = \textsc{waiting for outbound
        revocation}$}
          \State $i \gets i + 1$
      ...
      \Indent
        ...
        \If{$\textit{State}^P = \textsc{waiting for revocation}$}
          ...
          revocation}$
        \Else \: \Comment{$\textit{State}^P = \textsc{waiting for inbound
        revocation}$}
          \State $i \gets i + 1$
          \State $\textit{State}^P \gets \textsc{waiting for hosts ready}$
        \EndIf
    ...
    \begin{algorithmic}[1]
      ...
      \Else \: \Comment{real world}
        ...
        \Else \: \Comment{opening virtual channel}
          \State \remotecode{$\textit{State}^B \gets \textsc{waiting for
          check keys}$}
        \EndIf
    ...
    \begin{algorithmic}[1]
      ...
      \Else \: \Comment{opening virtual channel}
        ...
        \Else \: \Comment{real world}
          ...
          output (\textsc{funded}, $\texttt{host}_P$) \Comment{ignore any
          other message}
        \EndIf
      \EndIf
    \end{algorithmic}
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- external open messages for \bob}
    \begin{algorithmic}[1]
      ...
      \Indent
        \State ensure $\textit{State}^B = \textsc{waiting for check keys}$
        \State ensure $\pk{1} = \pk{A, F} \wedge \pk{2} = \pk{B, F}$
        \State $\textit{State}^B \gets \textsc{waiting for fudned}$
    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        the \textsc{virtualising} state} \Comment{we are the first hop}
          ...
          \Comment{do not expect reply by \texttt{funder}}
        \EndIf
  ...
  \begin{processbox}{\textsc{ln}.\textsc{revocationsTrip}()}
    \begin{algorithmic}[1]
      ...
      \texttt{host\_sibling}} \Comment{we are intermediary channel}
        \State input (\textsc{ew balance}, $c_P$, $c_{\bar{P}}$) to
        $\texttt{host}_P$
    ...
    \begin{algorithmic}[1]
      ...
      \If{ideal world}
        ...
        \texttt{host\_sibling}} \Comment{we are intermediary channel}
          ...
          balance ok})}
          \State \trustedcode{output (\textsc{new balance ok}) to
          \texttt{sibling}} \Comment{run by \textsc{virt}}
          \State \trustedcode{output (\textsc{new balance ok}) to
          \texttt{guest}} \Comment{run by \textsc{virt}}
        \EndIf
      \Else
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- On (\textsc{check chain for old comm}):}
    \begin{algorithmic}[1]
      ...
      \State send (\textsc{read}) to \ledger as $P$ and assign reply to $\Sigma$
