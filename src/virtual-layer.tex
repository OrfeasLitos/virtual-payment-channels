  ...
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \texttt{right\_virt}) \vee (\texttt{"3"} \wedge \texttt{right\_virt} +
        (t + s))$
  ...
  $R \in \{\texttt{guest}, \texttt{funder}, \texttt{fundee}\}$:}
    \begin{algorithmic}[1]
      ...
      \If{\texttt{funder} is defined}
        ...
        c_{\mathrm{guest}}$ coins and a $2/\{\pk{\mathrm{loc}, \mathrm{virt}},
        \pk{\mathrm{rem}, \mathrm{virt}}\}$ spending method with
        expired/non-existent timelock in $\Sigma$ \Comment{\texttt{guest}'s
        output}
        ...
        coins and a $2/\{\pk{\mathrm{left}, \mathrm{guest}}, \pk{\mathrm{right},
        \mathrm{guest}}\}$ spending method with expired/non-existent timelock in
        $\Sigma$ \Comment{\texttt{funder}'s output}
      ...
      \ElsIf{\texttt{fundee} is defined}
        ...
        coins and a $2/\{\pk{\mathrm{left}, \mathrm{guest}}, \pk{\mathrm{right},
        \mathrm{guest}}\}$ spending method with expired/non-existent timelock in
        $\Sigma$ \Comment{\texttt{fundee}'s output}
      \Else \: \Comment{we are intermediary}
        \State ensure that there exists an output with $c_P + c_{\bar{P}} -
        c_{\mathrm{guest}}$ coins and a $2/\{\pk{\mathrm{loc}, \mathrm{virt}},
        \pk{\mathrm{rem}, \mathrm{virt}}\}$ spending method with
        expired/non-existent timelock and an output with $c_{\mathrm{guest}}$
        coins and a $\pk{\mathrm{loc}, \mathrm{out}}$ spending method with
