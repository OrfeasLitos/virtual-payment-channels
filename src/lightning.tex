...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- open}
    \begin{algorithmic}[1]
      \State On (\textsc{open}, $c$, \texttt{fundee}, \texttt{hops}):
      \Comment{\texttt{fundee} is \bob}
      \Indent
        \State ensure $P = \alice$ \Comment{activated party is the funder}
        ...
        \Else \: \Comment{real world}
          ...
          \State \remotecode{ensure \textit{State} = \textsc{init}}
        ...
        \Else \: \Comment{opening virtual channel}
          ...
          reply (\textsc{virtual ok})
          \TODO{recheck that this works, make it work with \textsc{virt}}
          \TODO{use \texttt{left} similarly in \textsc{virt}}
        \EndIf
        \If{real world}
          \State $C_{A, 0} \gets$ TX \{input: $F$.output, outputs: ($c$,
          ($\pk{A, \mathrm{out}} + t$) $\vee$ $2/\{\pk{A, R},
          \pk{B, R}\}$), ($0$, $\pk{B, \mathrm{out}}$)\}
          \State $C_{B, 0} \gets$ TX \{input: $F$.output, outputs: ($c$, $\pk{A,
          \mathrm{out}}$), ($0$, ($\pk{B, \mathrm{out}} + t$) $\vee$ $2/\{\pk{A,
          R}, \pk{B, R}\}$)\}
          ...
          channel}
            \State \remotecode{$F \gets$ TX \{input: $(c, \pk{A,
            \mathrm{chain}})$, output: $(c, \texttt{3} \wedge 2/\{\pk{A, F},
            \pk{B, F}\})$\}}
          \Else \: \Comment{opening virtual channel}
            \State \remotecode{ensure we have received (\textsc{virtual ok}) by
            \texttt{hops}[-1].\texttt{right}}
          \EndIf
          \State \remotecode{$C_{B, 0} \gets$ TX \{input: $F$.output, outputs:
          ($c$, $\pk{A, \mathrm{out}}$), ($0$, ($\pk{B,
          ...
          ($c$, ($\pk{A, \mathrm{out}} + t$) $\vee$ $2/\{\pk{A, R}, \pk{B,
          R}\}$), ($0$, $\pk{B, \mathrm{out}}$)\}}
        ...
        \State $\textit{State} \gets \textsc{open}$
