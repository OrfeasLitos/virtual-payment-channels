...
\label{sec:short-related-work}
Perun~\cite{perun},
Thunderdome~\cite{avarikioti2025thunderdometimelockfreerationallysecurevirtual},
and GSCN~\cite{DBLP:conf/ccs/DziembowskiFH18} build virtual state channels on
the Turing-complete language of Ethereum, which cannot be translated to Bitcoin
due to its lower expressiveness.
  GSCN uses a per-channel functionality and a recursive argument similar
  to that of our UC-security analysis. However, their security argument is
  flawed, as they incorrectly argue that every level is subroutine respecting
  w.r.t.\ the same environment and subroutines
  (see~\cite{DBLP:conf/tcc/BadertscherCHTZ20} for a definition of subroutine
  respecting).
  ...
  Donner~\cite{donner}
    (released originally concurrently with the first technical report of Elmo) achieves variadic, but not recursive
  virtual channels. It is
  more efficient than Elmo in terms of storage, computation and communication,
  and boasts a simpler design.
%  is the first work 
%  to 
  ...
  Additionally, contrary to Elmo, a Donner channel needs active periodic
  collaboration of all channel parties to refresh
  it, thus Donner channels do not have an unlimited
  lifetime. We conjecture that using external coins precludes variadic
  virtual channels with unlimited lifetime. This
  further means that Donner is not symmetric. Donner also uses
  ...
  cost. Its lack of recursiveness mean that if a party with coins in a
  ...
  channel parties can fund the virtual channel.
  %, but has less room for optimisations and is not recursive.
  Their work also introduces \emph{Domino},
  which we discuss
  % acmart, long
  %in Sec.~\ref{sec:future-work}.
  % IEEE, short
  in~\cite{full-version}. We briefly mention
  here that, contrary to what is claimed in~\cite{donner}, Domino does not force
  the base channels of an Elmo virtual channel to close.

  We refer the reader
  % not IEEEtrans
  %to Appx.~\ref{sec:further-related-work}
  % IEEEtrans
  to~\cite{full-version}
  %
  for further
  ...
  work. Future work can be found
  % not IEEEtrans
  %in Appx.~\ref{sec:extra-future-work}.
  % IEEEtrans
