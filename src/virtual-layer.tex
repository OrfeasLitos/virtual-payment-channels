  ...
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \State $\texttt{bridge}_1 \gets 5/\{\pk{1, 2, 1}, \pk{i-1, 2, 1}, \pk{i,
        2, 1}, \pk{i+1, 2, 1}, \pk{n, 2, 1}\} \wedge \texttt{"1"}$ \Comment{We
        reuse the $\pk{j, 2, 1}$ keys for all \texttt{bridge}s to avoid new
        keys}
        \ForAll{$k \in \{m, \dots, l\} \setminus \{i\}$}
          \State $\texttt{bridge}_{2, k} \gets 5/\{\pk{1, 2, 1}, \pk{i-1, 2, 1},
          \pk{i, 2, 1}, \pk{i+1, 2, 1}, \pk{n, 2, 1}\} \wedge \texttt{"2,} k
          \texttt{"}$
        \EndFor
        \ForAll{$(k_1, k_2) \in \{m, \dots, i-1\} \times \{i+1, \dots, l\}$}
          \State $\texttt{bridge}_{3, k_1, k_2} \gets 5/\{\pk{1, 2, 1}, \pk{i-1,
          2, 1}, \pk{i, 2, 1}, \pk{i+1, 2, 1}, \pk{n, 2, 1}\} \wedge
          \texttt{"3,} k_1 \texttt{,} k_2 \texttt{"}$
        \EndFor
          ...
          \Indent
            ...
            \Indent
              ...
              \Indent
                ...
                \State (if $(n > 3)$ then $(\texttt{virt\_fund} + t_3)$
        ...
        \If{$i = n-1$}
          ...
          \Indent
            ...
            \Indent
              ...
              \Indent
                \State \texttt{revocation} $\vee$
                \State (if $(n - 2 > 1)$ then $(\texttt{virt\_fund} + t_{n-2})$
                \State else \texttt{virt\_fund})
              \EndIndent
              \State )
            \EndIndent
            ...
            \Indent
              ...
              \Indent
                ...
                \Indent
                  ...
                  \Indent
                    \State $\texttt{virt\_fund} + \max{(t_{k-1}, t_{i+1})}$
                  \EndIndent
                \EndIndent
                \State )
              \EndIndent
            ...
            \Indent
              ...
              \Indent
                ...
                \Indent
                  ...
                  \Indent
                    \State $\texttt{virt\_fund} + \max{(t_{i-1}, t_{k+1})}$
                  \EndIndent
                \EndIndent
                \State )
              \EndIndent
          ...
          \Indent
            ...
            \Indent
              ...
              \Indent
                ...
                \State $\vee \texttt{bridge}_{3, k_1, k_2}$
              \EndIndent
              \State )
            \EndIndent
          \EndIndent
  ...
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \EndIf
        \State $\texttt{bridge} \gets 3/\{\pk{1, 2, 1}, \pk{2, 2, 1}, \pk{n, 2,
        1}\} \wedge \texttt{"1"}$ \Comment{We reuse the $\pk{j, 2, 1}$ keys to
        avoid new keys}
        ...
        \Indent
          ...
          \Indent
            ...
            \State $(c_{\mathrm{virt}}, \mathit{all} \vee \texttt{bridge})$
...
\begin{center}
  \begin{processbox}{\textsc{virt.siblingSigs}()}
    \begin{algorithmic}[1]
      ...
      \end{itemize}
      \Indent
        \State sig($B_{i, 1}, \pk{1, 2, 1}$), sig($B_{i, 1}, \pk{i-1, 2, 1}$)
      \EndIndent
      \begin{itemize}
      ...
      \EndIndent
      \begin{itemize}
        \item \Comment{$2 \cdot (l-m)$ signatures}
      \end{itemize}
      \Indent
        \State $\forall k \in \{m, \dots, l\} \setminus \{i\}:$
        \Indent
          \State sig($B_{i, 2, k}, \pk{1, 2, 1}$), sig($B_{i, 2, k}, \pk{i-1, 2,
          1}$)
        \EndIndent
      \EndIndent
      \begin{itemize}
      ...
      \EndIndent
      \begin{itemize}
        \item \Comment{$2 \cdot (i-m) \cdot (l-i)$ signatures}
      \end{itemize}
      \Indent
        \State $\forall k_1 \in \{m, \dots, i-1\}, \forall k_2 \in \{i+1, \dots,
        l\}:$
        \Indent
          \State sig($B_{i, 3, k_1, k_2}, \pk{1, 2, 1}$), sig($B_{i, 3, k_1,
          k_2}, \pk{i-1, 2, 1}$)
        \EndIndent
      \EndIndent
      ...
      \If{$i+1 = n$} \Comment{next hop is \texttt{host\_fundee}}
        ...
        \texttt{ANYPREVOUT}) to $\mathrm{sigs}_{toRight}$
      \EndIf
      ...
      \end{itemize}
      \Indent
        \State sig($B_{i, 1}, \pk{i+1, 2, 1}$), sig($B_{i, 1}, \pk{n, 2, 1}$)
      \EndIndent
      \begin{itemize}
      ...
      \Indent
        \State $\forall k \in \{m, \dots, l\} \setminus \{i\}:$
        \Indent
          \State sig($B_{i, 2, k}, \pk{i+1, 2, 1}$), sig($B_{i, 2, k}, \pk{n, 2,
          1}$)
        \EndIndent
      \EndIndent
      \begin{itemize}
      ...
      \Indent
        ...
        l\}:$
        \Indent
          \State sig($B_{i, 3, k_1, k_2}, \pk{i+1, 2, 1}$), sig($B_{i, 3, k_1,
          k_2}, \pk{n, 2, 1}$)
        \EndIndent
      \EndIndent
  ...
  \begin{processbox}{\textsc{virt.intermediarySigs}()}
    \begin{algorithmic}[1]
      ...
      \If{$i = 2$} \Comment{previous hop is \texttt{host\_funder}}
        ...
        \texttt{ANYPREVOUT}) to $\mathrm{sigs}_{toLeft}$
      \EndIf
    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        maliciously published old virtual tx}
          ...
          \Sigma$}
            \State $(B, \sk{}) \gets (B_{i-1, 2, k}, \sk{i, i-1, k})$
          \Else
            \State $(B, \sk{}) \gets (B_{i-1, 1}, \sk{i, 1})$
          \EndIf
          \State input (\textsc{submit}, ($B$, $R_{\mathrm{loc}, \mathrm{left},
          \mathrm{virt}}$, $R_{\mathrm{loc}, \mathrm{left}, \mathrm{fund}}$),
          (\textsc{sign}($B$, $\sk{}$), \textsc{sign}($R_{\mathrm{loc},
          \mathrm{left}, \mathrm{virt}}$, $\sk{i, \mathrm{rev}}$),
          \textsc{sign}($R_{\mathrm{loc}, \mathrm{left}, \mathrm{fund}}$,
          $\sk{i, \mathrm{rev}}$))) to \ledger
        \EndIf
        \If{$\mathrm{TX}_{i+1, 1} \in \Sigma \vee \exists k \in \mathbb{N}:
        \mathrm{TX}_{i+1, 2, k} \in \Sigma$} \Comment{right
        counterparty maliciously published old virtual tx}
          ...
          \Else
            \State $(B, \sk{}) \gets (B_{i+1, 1}, \sk{i, 1})$
          \EndIf
          \State input (\textsc{submit}, ($B$, $R_{\mathrm{loc}, \mathrm{right},
          \mathrm{virt}}$, $R_{\mathrm{loc}, \mathrm{right}, \mathrm{fund}}$),
          (\textsc{sign}($B$, $\sk{}$), \textsc{sign}($R_{\mathrm{loc},
          \mathrm{right}, \mathrm{virt}}$, $\sk{i, \mathrm{rev}}$),
          \textsc{sign}($R_{\mathrm{loc}, \mathrm{right}, \mathrm{fund}}$,
          $\sk{i, \mathrm{rev}}$))) to \ledger
        \EndIf
...
\begin{center}
  \begin{processbox}{\textsc{virt} -- On input (\textsc{forceClose}) by $R$:}
    \begin{algorithmic}[1]
      ...
      \State $\texttt{txs} \gets (\mathrm{TX}_p, B_p)$
      \State let \texttt{sigs} be the set of stored signatures that sign
      either $\mathrm{TX}_p$ or $B_p$
      ...
      \sk{i, j, k}, \texttt{ANYPREVOUT}))_{j, k \in [n]}$, $(\textsc{sign}(B_p,
      ...
      \State $\textit{State} \gets \textsc{closing}$
      \State send (\textsc{submit}, \texttt{txs}, \texttt{sigs}) to \ledger
    \end{algorithmic}
  \end{processbox}
  \captionof{figure}{}
