  ...
  \begin{processbox}{\textsc{ln} -- virtualise}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        V}$, ($\sk{P, F}'$, $\pk{P, F}'$), $\pk{\bar{P}, F}'$) to
        $\texttt{host}_P'$ and expect reply (\textsc{hosts ready})
      ...
      \Indent
        ...
        \If{$\mathrm{len}(\texttt{hops}) > 1$} \Comment{we are not the last hop}
          ...
          $c$) to $\texttt{host}_P'$ and expect reply (\textsc{host init ok})
      ...
      \Indent
        ...
        \State input (\textsc{init}, $\texttt{host}_P$,
        $\texttt{host}_{\bar{P}}'$, \texttt{host\_sibling}, ($\sk{P, F}'$,
        $\pk{P, F}'$), $\pk{\bar{P}, F}'$, $\pk{\mathrm{sib}, \bar{P}, F}$, $c$)
