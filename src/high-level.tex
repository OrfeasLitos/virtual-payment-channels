...
\subsection{Virtual Channels}
  ...
  If the
  initiator 
  %\TODO{initiator?} \orfeas{Yes. Each endpoint has only one virtual
  % transaction, the initiator} ok fixed
...
%  \TODO{we need to add here a couple of sentences about how updates are  performed between the two end-points in the virtual channels.} %\orfeas{check  next sentence} All ok.
  ...
  stemming from Proposition~\ref{prop:liveness} of Appendix \TODO{define windowsize briefly}),  or $p +
  ...
  subexponential number of off-chain transactions for each party cannot be constructed in
  ...
  of messages are exchanged \TODO{you mean $6$ rounds of communication?}, starting from the funder and hop by hop reaching
