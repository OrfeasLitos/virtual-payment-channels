  ...
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \textsc{keyGen}()$
        \If{$\textit{pk}_{\mathrm{left}}$ is given as argument} \Comment{we are
        not \texttt{host\_funder}}
          ...
          \texttt{host\_fundee}}
            ...
            \Comment{sibling}
            \State \trustedcode{store $\textit{pk}_{\mathrm{left}},
            (\textit{sk}_{\mathrm{loc}}, \textit{pk}_{\mathrm{loc}})$}
      ...
      \State \textsc{circulateVirtualSigs}($c$, \texttt{hops}, $(\textit{pk}_{A,
      V}, \textit{pk}_{B, V})$, $\mathrm{sigs}_{\mathrm{byLeft}}$):
      \Indent
