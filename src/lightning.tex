  ...
  \begin{processbox}{\textsc{ln.prepareBase}()}
    \begin{algorithmic}[1]
      ...
      \Else \: \Comment{opening virtual channel}
        ...
        $t_P$) \TODO{added $t_P$ -- decide how to get informed of $t_P$} \Comment{ignore any other message}
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln.exchangeOpenSigs}()}
    \begin{algorithmic}[1]
      ...
      \State $C_{A, 0} \gets$ TX \{input: $(c, \texttt{"3"} \wedge 2/\{\pk{A,
      F}, \pk{B, F}\})$, outputs: ($c$, ($\pk{A, \mathrm{out}} + (t + s)$)
      $\vee$ $2/\{\pk{A, R}, \pk{B, R}\}$), ($0$, $\pk{B, \mathrm{out}}$)\}
      \label{code:ln:exchange-open-sigs:a-tx}
      \State $C_{B, 0} \gets$ TX \{input: $(c, \texttt{"3"} \wedge 2/\{\pk{A,
      F}, \pk{B, F}\})$, outputs: ($c$, $\pk{A, \mathrm{out}}$), ($0$, ($\pk{B,
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- external open messages for \bob}
    \begin{algorithmic}[1]
      ...
      \TODO{added $t_P$ -- decide how to get informed of $t_P$}
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln}.\textsc{updateForVirtual}()}
    \begin{algorithmic}[1]
      ...
      respectively, reducing the input and $P$'s output by $c_{\mathrm{guest}}$
      ...
      reducing the input and $P$'s output by $c_{\mathrm{guest}}$}
      ...
      the input and $P$'s output by $c_{\mathrm{guest}}$
  ...
  \begin{processbox}{\textsc{ln} -- virtualise start and end}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \texttt{hops}, \texttt{fundee}, $c_{\mathrm{guest}}$, $2$,
        ...
        $c_{\mathrm{guest}}$, $\pk{A, V}$, $\pk{B, V}$, ($\sk{P, F}'$, $\pk{P,
    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \If{$\mathrm{len}(\texttt{hops}) > 1$} \Comment{we are not the last hop}
          ...
          \texttt{hops}[1:], \texttt{fundee}, $c_{\mathrm{guest}}$,
          ...
          $c_P$, $c_{\bar{P}}$, $c_{\mathrm{guest}}$, $i$, ``left'', $n$) to
        ...
        \Else \: \Comment{we are the last hop}
          ...
          $c_{\mathrm{guest}}$, $i$, ``left'', $n$) to new \textsc{virt} ITI
          $\texttt{host}_P'$ and expect reply (\textsc{host init ok})
        \EndIf
        \State $\itistate \gets \textsc{waiting for revocation}$
      ...
      \mathrm{out}}$, \texttt{hops}, \texttt{fundee}, $c_{\mathrm{guest}}$,
      ...
      \Indent
        ...
        \State define new \textsc{virt} ITI $\texttt{host}_P'$
        \State send (\textsc{virtualising}, $\texttt{host}_P'$, $\pk{P, F}'$,
        \texttt{hops}, \texttt{fundee}, $c_{\mathrm{guest}}$, $i+1$, $n$) to
        ...
        $\pk{P, F}'$), $\pk{\bar{P}, F}'$, $\pk{\mathrm{sib}, \bar{P}, F}$,
        ($\sk{P, F}$, $\pk{P, F}$), $\pk{\bar{P}, F}$, $\pk{P, \mathrm{out}}$,
        $c_{\mathrm{guest}}$) to $\texttt{host}_P'$ and expect reply
        (\textsc{host init ok})
        \State $\itistate \gets \textsc{waiting for revocation}$
  ...
  \begin{processbox}{\textsc{ln}.\textsc{signaturesRoundTrip}()}
    \begin{algorithmic}[1]
      ...
      has a \texttt{host\_sibling}}} \Comment{we are intermediary channel}
        \State \remotecode{ensure $c_{\bar{P}} + x \leq c_{\bar{P},
        \mathrm{sib}, \mathrm{rem}} \wedge c_{\bar{P}, \mathrm{sib}} \leq c_P -
        x$} \Comment{avoid value loss by griefing attack}
      \EndIf
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- On (\textsc{get paid}, $y$):}
    \begin{algorithmic}[1]
      \State ensure $\itistate = \textsc{open} \wedge c_{\bar{P}} \geq x$
      \State store $y$
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- On (\textsc{check chain for closed}):}
    \begin{algorithmic}[1]
      ...
      \ElsIf{$C_{P, j} \in \Sigma \wedge C_{\bar{P}, j} \in \Sigma$}
        \State $\itistate \gets \textsc{closed}$
