...
\begin{center}
  \begin{processbox}{\textsc{ln} -- On (\textsc{coop close}) by $R$:}
    ...
    \begin{algorithmic}[1]
      ...
      i+2}$) to $\bar{P}$
      \State \TODO{expect (\textsc{coop close ok}) by $\texttt{host}_P$}
    \end{algorithmic}
  \end{processbox}
  \captionof{figure}{}
...
\begin{center}
  ...
  $\texttt{sig\_bal}_{\bar{P}}$) by $\bar{P}$:}
    \begin{algorithmic}[1]
      ...
      \State $\mathrm{sig}_{1, \mathrm{right}, \mathrm{rev}} \gets
      \textsc{sign}(R_{\mathrm{rem}, \mathrm{right}}, \sk{1, \mathrm{rev}})$
      \ForAll{$j \in \{2, \dots, n\}$}
        ...
        \pk{j, \mathrm{out}})\}$
        \State $\mathrm{sig}_{1, j, \mathrm{left}, \mathrm{rev}} \gets
        \textsc{sign}(R_{j, \mathrm{left}}, \sk{1, \mathrm{rev}})$
      \EndFor
