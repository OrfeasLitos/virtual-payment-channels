\section{Security}
  ...
  % not IEEEtrans
  %(\ref{lemma:real-balance-security} and~\ref{lemma:ideal-balance})
  %
  along with all proofs are deferred
  % not IEEEtrans
  %to Appx.~\ref{sec:proofs}.
  % IEEEtrans
...
\label{lemma:real-balance-security-informal}
  ...
  \end{itemize}
  If $P$ performs a successful unilateral close, eventually there will be $h \in \mathbb{N}^*$ outputs on-chain
...
\end{lemma}
  ...
  payments]. A
  similar guarantee is provided for the ideal-world functionality
  % not IEEEtrans
...
\end{proofsketch}
    It is important to note that in fact our protocol \pchan
    % not IEEEtrans
    %(Appx.~\ref{sec:protocol})
    provides a stronger guarantee:
  a party can always successfully unilaterally close its channel
  ...
  % not IEEEtrans
  %\fchan (Appx.~\ref{sec:functionalities})
  % IEEEtrans
  \fchan~\cite{full-version}
  %
  halts on security breaches (e.g., lower
  than expected balance).
  We prove the ``no-halt'' Lemma
  % not IEEEtrans
  %(\ref{lemma:no-halt}),
  %
