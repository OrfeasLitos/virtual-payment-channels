    ...
    \begin{algorithmic}[1]
      ...
      \Else \: \Comment{opening virtual channel}
        ...
        \Else \: \Comment{real world}
          ...
          output (\textsc{hosts ready}, $\texttt{host}_P$) \Comment{ignore any
          other message}
        \EndIf
      \EndIf
    \end{algorithmic}
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- external open messages for \bob}
    \begin{algorithmic}[1]
      ...
      \State On output (\textsc{hosts ready}, $\texttt{host}_P$) by
  ...
  \begin{processbox}{\textsc{ln} -- virtualise}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        $\texttt{host}_{\bar{P}}'$, $\texttt{host}_P$, $c$, $\pk{A, V}$, $\pk{B,
        V}$, ($\sk{P, F}'$, $\pk{P, F}'$), ($\sk{P, F}$, $\pk{P, F}$),
        $\pk{\bar{P}, F}'$) to $\texttt{host}_P'$ and expect reply
        (\textsc{hosts ready})
      ...
      \Indent
        ...
        \If{$\mathrm{len}(\texttt{hops}) > 1$} \Comment{we are not the last hop}
          ...
          ($\sk{P, F}$, $\pk{P, F}$), $c$) to $\texttt{host}_P'$ and expect
          reply (\textsc{host init ok})
        \Else \: \Comment{we are the last hop}
          \State input (\textsc{init}, $\texttt{host}_P$,
          $\texttt{host}_{\bar{P}}'$, ($\sk{P, F}'$, $\pk{P, F}'$),
          $\pk{\bar{P}, F}'$, ($\sk{P, F}$, $\pk{P, F}$), $c$) to new
          \textsc{virt} ITI $\texttt{host}_P'$ and expect reply (\textsc{host
          init ok})
        \EndIf
      ...
      \Indent
        ...
        $\texttt{host}_{\bar{P}}'$, \texttt{host\_sibling}, ($\sk{P, F}'$,
        $\pk{P, F}'$), $\pk{\bar{P}, F}'$, $\pk{\mathrm{sib}, \bar{P}, F}$,
        ($\sk{P, F}$, $\pk{P, F}$), $c$) to $\texttt{host}_P'$ and expect reply
        (\textsc{host init ok})
        \State $\textit{State}^P \gets \textsc{waiting for hosts ready}$
