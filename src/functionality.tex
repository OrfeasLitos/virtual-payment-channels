  ...
  \begin{systembox}{\fchan{} -- state machine, Pt. 1}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \If{$\texttt{ignore}_{\bar{P}} = \mathrm{True}$}
          \State replace all actions in
          Fig.~\ref{code:functionality:state-machine-1}
          and~\ref{code:functionality:state-machine-2} with no-op \Comment{if
          both parties are corrupted, \fchan deactivates its state machine and
          does not do the balance check -- it becomes a simple relay between
      ...
      \Indent
        \If{$\texttt{ignore}_A =
        \texttt{ignore}_B = \mathrm{False}$}
          \State ensure $\itistate = \textsc{tentative-open}$
        \ElsIf{$\texttt{ignore}_A = \text{True} \wedge \texttt{ignore}_B =
        \text{False}$}
          \State ensure $\itistate \in \{\textsc{init}_B,
          \textsc{tentative-open}\}$
        \ElsIf{$\texttt{ignore}_A = \text{False} \wedge \texttt{ignore}_B =
        \text{True}$}
