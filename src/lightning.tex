    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \textsc{closed}$} \Comment{channel is open}
          ...
          \EndIf
        \EndIf
        \If{\itistate = \textsc{waiting for nothing revoked} $\wedge$ activation
        is not caused by output (\textsc{nothing revoked}), received by a member
        of the list of old hosts} \Comment{the only way for this case to be true
        is if the old host punished a misbehaving counterparty}
      ...
      \Indent
        ...
        \State $\texttt{hosting} \gets$ False
  ...
  \begin{processbox}{\textsc{ln} -- On (\textsc{check chain for closed}):}
    ...
    \begin{algorithmic}[1]
      ...
      \ElsIf{$C_{P, i} \in \Sigma \vee C_{\bar{P}, i} \in \Sigma$}
        ...
        \State output (\textsc{closed})
      \Else
        \State $\texttt{state\_before\_checking\_revoked} \gets \itistate$
        \For{\textbf{each} \texttt{host} in list of old hosts}
          \State $\itistate \gets \textsc{waiting for nothing revoked}$
          \State input (\textsc{check for revoked}) to \texttt{host} and expect
          output (\textsc{nothing revoked})
          \label{code:ln:poll:hosts-poll}
          \State $\itistate \gets \texttt{state\_before\_checking\_revoked}$
        \EndFor
      \EndIf
    \end{algorithmic}
  \end{processbox}
  ...
  \begin{processbox}{\textsc{ln} -- On (\textsc{forceClose}):}
    ...
    \begin{algorithmic}[1]
      ...
      \Else \: \Comment{counterparty is idle}
        \While{$\neg\exists$ unspent \texttt{output} $\in \Sigma$ that $C_{P, i}$
...
\begin{center}
  \begin{processbox}{\textsc{ln} -- On (\textsc{coopClose}):}
    ...
    \begin{algorithmic}[1]
      ...
      \If{\texttt{hosting} = True $\vee$ we have received \textsc{open} from
      $\environment$ while $\itistate$ was \textsc{topped up}} \Comment{we are
      not the \texttt{fundee} of a channel that is not the base of any other
      channel}
        \If{\texttt{hosting} = True} \Comment{we are not the \texttt{funder} of
        ...
        \Else \: \Comment{we are the \texttt{funder} of the channel to be
        closed}
          \State the next time we are activated, if we are not activated by
      ...
      \Else \: \Comment{we are the \texttt{fundee} of a channel that is not the
      base of any other channel}
...
\begin{center}
  \begin{processbox}{\textsc{ln} -- On (\textsc{coopClosed}) by $R$:}
    \ \\
    \begin{algorithmic}[1]
      \If{\texttt{hosting} = True} \Comment{we are intermediary}
        \State ensure $\itistate = \textsc{open}$
...
\begin{center}
  \begin{processbox}{\textsc{ln} -- On (\textsc{coop close}) by $R$:}
    ...
    cooperatively by \environment -- cf. Fig.~\ref{code:ln:coop-close},
...
\begin{center}
  \begin{processbox}{\textsc{ln} -- On (\textsc{coop close},
  $\texttt{sig\_bal}_{\bar{P}}$) by $\bar{P}$:}
    \ \\
    \begin{algorithmic}[1]
      \State ensure we are \texttt{funder}
      \State ensure $\itistate = \textsc{open}$
      \State parse $\texttt{sig\_bal}_{\bar{P}}$ as $((c_1', c_2'),
      \mathrm{sig}_{\bar{P}})$
      \State ensure $c_P = c_1' \wedge c_{\bar{P}} = c_2' \wedge
      \textsc{verify}((c_1', c_2'), \mathrm{sig}_{\bar{P}}, \pk{\bar{P}, F}) =
      \mathrm{True}$
      \label{code:ln:coop-close-funder:verify-balance}
      \State $\texttt{sig\_bal} \gets ((c_P, c_{\bar{P}}), \textsc{sign}((c_P,
      c_{\bar{P}}), \sk{P, F}), \mathrm{sig}_{\bar{P}})$
      \State $\itistate \gets \textsc{coop closing}$
      \label{code:ln:coop-close-funder:state-change}
      \State input (\textsc{coop close}, $\texttt{sig\_bal}$) to
      $\texttt{host}_P$
      \State \trustedcode{ensure $\itistate = \textsc{open}$} \Comment{executed
      by $\texttt{host}_P$}
      \State \trustedcode{$\itistate \gets \textsc{coop closing}$}
      \State \trustedcode{output (\textsc{coop close sign comm funder}, $(c_1',
      c_2')$) to \texttt{guest}}
      \State \trustedcode{ensure $\itistate = \textsc{open}$} \Comment{executed
      by \texttt{guest} of $\texttt{host}_P$}
      \State \trustedcode{$\itistate \gets \textsc{coop closing}$}
      \label{code:ln:coop-close-funder:guest-state-change}
      \State \trustedcode{remove most recent keys from list of old funding keys
      and assign them to $\sk{P, F}'$, $\pk{P, F}'$, $\pk{\bar{P}, F}'$}
      ...
      \State \trustedcode{rename received signature to $\mathrm{sig}_{1,
      \mathrm{right}, C}$} \Comment{executed by $\texttt{host}_P$}
      \State \trustedcode{rename received public key to $\pk{1, \mathrm{right},
      R}$}
      \State \trustedcode{send (\textsc{coop close}, \texttt{sig\_bal},
      $\mathrm{sig}_{1, \mathrm{right}, C}$, $\pk{1, \mathrm{right}, R}$) to
      $\bar{P}$ and expect reply (\textsc{coop close back},
      (\texttt{right\_comms\_revkeys}, \texttt{right\_revocations})}
      \State \trustedcode{$R_{\mathrm{loc}, \mathrm{virt}} \gets$ TX \{input:
      $(c_{\mathrm{virt}}, 2/\{\pk{1, \mathrm{rev}}, \pk{2, \mathrm{rev}}\})$,
      output: $(c_{\mathrm{virt}}, \pk{1, \mathrm{out}})$\}}
      \State \trustedcode{extract $\mathrm{sig}_{2, \mathrm{right},
      \mathrm{rev}, \mathrm{virt}}$ from \texttt{right\_revocations}}
      \State \trustedcode{ensure \textsc{verify}($R_{\mathrm{loc},
      \mathrm{virt}}$, $\mathrm{sig}_{2, \mathrm{right}, \mathrm{rev},
      \mathrm{virt}}$, $\pk{2, \mathrm{rev}}$) = True}
      \State \trustedcode{$R_{\mathrm{loc}, \mathrm{fund}} \gets$ TX \{input:
      $(c_P + c_{\bar{P}}, 2/\{\pk{1, \mathrm{rev}}, \pk{2, \mathrm{rev}}\})$,
      output: $(c_P + c_{\bar{P}}, \pk{1, \mathrm{out}})$\}}
      \State \trustedcode{extract $\mathrm{sig}_{2, \mathrm{right},
      \mathrm{rev}, \mathrm{fund}}$ from \texttt{right\_revocations}}
      \State \trustedcode{ensure \textsc{verify}($R_{\mathrm{loc},
      \mathrm{fund}}$, $\mathrm{sig}_{2, \mathrm{right}, \mathrm{rev},
      \mathrm{fund}}$, $\pk{2, \mathrm{rev}}$) = True}
      \State \trustedcode{extract $\mathrm{sig}_{2, \mathrm{right}, R}$ from
      \texttt{right\_revocations}}
      \State \trustedcode{extract $\mathrm{sig}_{2, \mathrm{right}, C}$ from
      \texttt{right\_comms\_revkeys}}
      \State \trustedcode{extract $\pk{2, R}$ from
      \texttt{right\_comms\_revkeys}}
      \State \trustedcode{output (\textsc{verify revoke}, $\mathrm{sig}_{2,
      \mathrm{right}, C}$, $\mathrm{sig}_{2, \mathrm{right}, R}$, $\pk{2, R}$,
      $\texttt{host}_P$) to \texttt{guest}}
      \State \trustedcode{store $\mathrm{sig}_{2, \mathrm{right}, C}$ as
      $\mathrm{sig}_{\bar{P}, C, i+1}$} \Comment{executed by \texttt{guest} of
      $\texttt{host}_P$}
      \State \trustedcode{store $\mathrm{sig}_{2, \mathrm{right}, R}$ as
      ...
      c_1' + c_2')$, outputs: $(c_P + c_1', (\pk{P, \mathrm{out}} + (p + s)) \vee
      2/\{\pk{P, R, i+1}, \pk{\bar{P}, R, i+1}\})$, $(c_{\bar{P}} + c_2',
      \pk{\bar{P}, \mathrm{out}})$\}}
      ...
      \State \trustedcode{input (\textsc{verified}) to $\texttt{host}_P$}
      \State \trustedcode{extract $\mathrm{sig}_{n, \mathrm{left}, R}$ from
      \texttt{right\_revocations}} \Comment{executed by $\texttt{host}_P$}
      \State \trustedcode{output (\textsc{verify revocation}, $\mathrm{sig}_{n,
      \mathrm{left}, R}$) to \texttt{funder}}
      \State $R_{P, i} \gets$ TX \{input: $C_{\bar{P},
      i}$.outputs.$\bar{P}$, output: $(c_{\bar{P}}, \pk{P, \mathrm{out}})$\}
      \State ensure \textsc{verify}($R_{P, i}, \mathrm{sig}_{\bar{P}, R, i},
      \pk{\bar{P}, R, i}$) = True
      \State $R_{\bar{P}, i} \gets$ TX \{input: $C_{P, i}$.outputs.$P$,
      output: $(c_P, \pk{\bar{P}, \mathrm{out}})$\}
      \State $\mathrm{sig}_{P, R, i} \gets \textsc{sign}(R_{\bar{P}, i}, \sk{P,
      R, i})$
      \State $\itistate \gets \textsc{coop closed}$ \Comment{in \textsc{ln},
      only virtual channels can end up in this state}
      \label{code:ln:coop-close-funder:coop-closed-state}
      \State input (\textsc{coop close revocation}, $\mathrm{sig}_{P, R, i}$) to
      $\texttt{host}_P$
      ...
      i}$.outputs.$P$, output: ($c_P$, $\pk{\bar{P}, \mathrm{out}}$)\}}
      \Comment{executed by \texttt{guest} of $\texttt{host}_P$}
      ...
      old enable channel funding keys}
      \State \trustedcode{add $\texttt{host}_P$ to list of old hosts}
      \State \trustedcode{assign received host to $\texttt{host}_P$}
      \State \trustedcode{$c_P \gets c_P + c_1'; c_{\bar{P}} \gets c_{\bar{P}} +
      c_2'$}
      \label{code:ln:coop-close-funder:update-coins}
      ...
      \State \trustedcode{input (\textsc{revocation},
      $\mathrm{sig}_{P, R, i}$) to last old host}
      \State \trustedcode{rename received signature to $\mathrm{sig}_{1,
      \mathrm{right}, R}$} \Comment{executed by $\texttt{host}_P$}
      \State \trustedcode{$R_{\mathrm{rem}, \mathrm{virt}} \gets$ TX \{input:
      ($c_{\mathrm{virt}}, 4/\{\pk{1, \mathrm{rev}}, \pk{1, \mathrm{rev}},
      \pk{2, \mathrm{rev}}, \pk{n, \mathrm{rev}}\}$), output:
      $(c_{\mathrm{virt}}, \pk{2, \mathrm{out}})$\}}
      \State \trustedcode{$\mathrm{sig}_{1, \mathrm{right}, \mathrm{rev},
      \mathrm{virt}} \gets \textsc{sign}(R_{\mathrm{rem}, \mathrm{virt}}, \sk{1,
      \mathrm{rev}})$}
      \State \trustedcode{$R_{\mathrm{rem}, \mathrm{fund}} \gets$ TX \{input:
      ($c_P + c_{\bar{P}}, 2/\{\pk{1, \mathrm{rev}}, \pk{2, \mathrm{rev}}\}$),
      output: $(c_P + c_{\bar{P}}, \pk{2, \mathrm{out}})$\}}
      \State \trustedcode{$\mathrm{sig}_{1, \mathrm{right}, \mathrm{rev},
      \mathrm{fund}} \gets \textsc{sign}(R_{\mathrm{rem}, \mathrm{fund}}, \sk{1,
      \mathrm{rev}})$}
      \ForAll{\trustedcode{$j \in \{2, \dots, n\}$}}
        \State \trustedcode{$R_{j, \mathrm{left}} \gets$ TX \{input:
        $(c_{\mathrm{virt}}, 4/\{\pk{1, \mathrm{rev}}, \pk{j-1, \mathrm{rev}},
        \pk{j, \mathrm{rev}}, \pk{n, \mathrm{rev}}\})$, output:
        $(c_{\mathrm{virt}}, \pk{j, \mathrm{out}})\}$}
        \State \trustedcode{$\mathrm{sig}_{1, j, \mathrm{left}, \mathrm{rev}}
        \gets \textsc{sign}(R_{j, \mathrm{left}}, \sk{1, \mathrm{rev}})$}
      \EndFor
      \State \trustedcode{$\itistate \gets \textsc{coop closed}$}
      \State \trustedcode{send (\textsc{coop close revocations},
      ($\mathrm{sig}_{1, \mathrm{right}, R}$, $\mathrm{sig}_{1, \mathrm{right},
      \mathrm{rev}, \mathrm{virt}}$, $\mathrm{sig}_{1, \mathrm{right},
      \mathrm{rev}, \mathrm{fund}}$, $(\mathrm{sig}_{1, j, \mathrm{left},
      \mathrm{rev}})_{j \in \{2, \dots, n\}}$)) to $\bar{P}$}
    \end{algorithmic}
  \end{processbox}
  \captionof{figure}{}
...
\begin{center}
  \begin{processbox}{\textsc{ln} -- On (\textsc{corrupt}) by \adversary or
  kindred party $R$:}
    \Comment{This is executed by the shell -- cf.~\cite{uc}}
    \begin{algorithmic}[1]
      \If{$\itistate \neq \textsc{corrupted}$}
        \State $\itistate \gets \textsc{corrupted}$
        \For{$S \in$ set of kindred parties}
          \State input (\textsc{corrupt}) to $S$ and expect reply (\textsc{ok})
        \EndFor
      \EndIf
      \State reply (\textsc{ok})
    \end{algorithmic}
  \end{processbox}
  \captionof{figure}{}
  \label{code:ln:shell-corrupt}
\end{center} \ \\
