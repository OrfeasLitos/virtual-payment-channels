...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- base}
    \begin{algorithmic}[1]
      \State \textsc{getCommTX}($c_A$, $c_B$, $\pk{A, F}$, $\pk{B, F}$, $\pk{A,
      \mathrm{out}}$, $\pk{B, \mathrm{out}}$, $\pk{A, R}$, $\pk{B, R}$):
      \Indent
      ...
      \Indent
        ...
        \If{real world}
          ...
          ($c_{\mathrm{chain}}$, ($\pk{A, \mathrm{out}} + t$) $\vee$ ($\pk{A,
          R}$ $\wedge$ $\pk{B, R}$)), ($0$, $\pk{B, \mathrm{out}}$)\}
          \State $C_{B, 0} \gets$ TX \{input: $F$.output, outputs:
          ($c_{\mathrm{chain}}$, $\pk{A, \mathrm{out}}$), ($0$, ($\pk{B,
          \mathrm{out}} + t$) $\vee$ ($\pk{A, R}$ $\wedge$ $\pk{B, R}$))\}
...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln} -- open virtual}
    \begin{algorithmic}[1]
      ...
      ANYPREVOUT?)}
      \Indent
        \State ensure \texttt{funding\_output} = 2/\{$\pk{A, F}, \pk{B, F}$\}
        \If{real world}
