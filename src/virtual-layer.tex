...
\begin{figure}[H]
  \begin{processbox}{\textsc{virt} -- close}
    \begin{algorithmic}[1]
      ...
      ignored}
      \label{code:virtual-layer:close:when-punished-1}
      \Indent
        ...
        \IfThen{\texttt{funder} or \texttt{fundee} is defined}{output
        (\textsc{enabler used revocation}) to it}
      \EndIndent
      \Statex

      \State On output (\textsc{used revocation}) by $\texttt{host}_P$:
      \label{code:virtual-layer:close:when-punished-2}
      \Indent
