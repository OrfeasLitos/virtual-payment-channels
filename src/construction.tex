...
\subsection{\textsc{virt} subprotocol}
  ...
  total of $nS \in O(n^4)$ signatures in this phase. The number of virtual
  transactions is $1$ for the two endpoints
  (Figure~\ref{code:virtual-layer:endpoint-txs}) and $\chi_{i = 2} + \chi_{i = n
  - 1} + n - 2 + (i - 2 + \chi_{i = 2}) (n - 1 + \chi_{i = n-1} - i) \in O(n^2)$
  (Figure~\ref{code:virtual-layer:mid-txs}). The third circuit concerns sharing
  signatures for the funding outputs
  ...
  initiator transaction and extend-interval transactions of its neighbours. The
  two endpoints send $2$ signatures each when $n = 3$ and $n - 2$ signatures
  each when $n > 3$, whereas each intermediary sends $1 + \chi_{i + 1 < n}(n - 2
  + \chi_{i = n - 2}) + 1 + \chi_{i - 1 > 1}(n - 2 + \chi_{i = 3}) \in O(n)$
  signatures each.
