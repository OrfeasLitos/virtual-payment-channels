...
Closing a non-virtual channel is an operation that involves posting the state of the channel
...
We call the two parties that share the channel \emph{endpoints} and the parties
of any of the underlying channels \emph{intermediaries}.
...
start closing the channel, while subsequent respondents, following the activation sequence, can choose the right action to complete the closure process by posting a single transaction each.
  ...
  Composition (UC)~\cite{uc} setting; our ideal functionality \fchan represents
  a single channel and is thus a global functionality,
  as defined in~\cite{DBLP:conf/tcc/BadertscherCHTZ20} (cf.
  Sec.~\ref{section:security}). Elmo requires the \texttt{ANYPREVOUT} signature type (slated for
