\begin{figure}[H]
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      the funder}
      \State call \textsc{pcn.virtualKeys}($c$, \texttt{fundee},
      \texttt{host\_funder}, \texttt{host\_fundee}) of \texttt{funder} and
      assign result to $(pk_{A, V}, pk_{B, V})$
      \State $((pk_{j, V})_{j \in [n]}, (C_j)_{j \in [n]}) \gets$
      \textsc{virt.hostInitData}(\texttt{hops}) \TODO{also return funding TX output of each hop and input
      those outputs to \textsc{virt.getTXs}()}
      \State $(T_{i,j})_{i \in [n], j \in [n+2]} \gets$
      \textsc{virt.getTXs}($c$, $pk_{A, V}$, $pk_{B, V}$, $(pk_{i, V})_i$,
      $(C_j)_{j \in [n]}$) \Comment{for $i \in \{1, n\}$, $j$ can only be equal
      to $1$}
...
\begin{figure}[H]
  \begin{notitlebox}{\textsc{virt.hostInitData}($C_i$, $c$, \texttt{hops})}
    \begin{algorithmic}[1]
      \State \Comment{can be implemented with \alice as leader or with 2 direct
      messages between all hops ($O(n^2)$)} \TODO{decide if we need to move to
      \textsc{updating} state here}
      \For{$i$ from $1$ to $n$} \Comment{one iteration executed by each hop}
        \If{$i \neq 1$}
          \State ensure $\texttt{hops}[i-1] =$ sender $\wedge \:
          \texttt{hops}[i] =$ self
        \EndIf
        \State $(sk_{i, V}, pk_{i, V}) \gets \textsc{keyGen}()$
        \If{real world}
          \State send (\textsc{hostDataForward}, \texttt{hops}, $(pk_{j, V})_{j
          \in \{1, \dots, i\}}$, $(C_j)_{j \in \{1, \dots, i\}}$) to
          \texttt{hops}[$i+1$] \Comment{$C_j$ is the lastest commitment TX of
          the $j$-th hop, not our commitment TX for the $j$-th update}
        \Else \: \Comment{ideal world}
          \If{\texttt{hops}[$i+1$] is honest}
            \State send (\textsc{hostDataForward}, \texttt{hops}, $(pk_{j,
            V})_{j \in \{1, \dots, i\}}$, $(C_j)_{j \in \{1, \dots, i\}}$,
            \texttt{hops}[$i+1$]) to \adversary, expecting reply (\textsc{ok})
          \Else \: \Comment{\texttt{hops}[$i+1$] is corrupted}
            \State send (\textsc{hostDataForward}, \texttt{hops}, $(pk_{j,
            V})_{j \in \{1, \dots, i\}}$ $(C_j)_{j \in \{1, \dots, i\}}$,,
            \texttt{hops}[$i+1$]) to \adversary, expecting reply
            (\textsc{hostDataForward}, \texttt{hops}, $(pk_{j, V})_{j \in \{1,
            \dots, k-1\}}$, $(C_j)_{j \in \{1, \dots, i\}}$, \texttt{hops}[$k$])
      ...
      \For{$i$ from $n$ down to $1$} \Comment{one iteration executed by each hop}
        ...
        \EndIf
        \If{real world}
          \State send (\textsc{hostDataBackward}, \texttt{hops}, $(pk_{j, V})_{j
          \in \{i, \dots, n\}}$, $(C_j)_{j \in \{i, \dots, n\}}$) to
          \texttt{hops}[$i-1$]
        \Else \: \Comment{ideal world}
          \If{\texttt{hops}[$i-1$] is honest}
            \State send (\textsc{hostDataBackward}, \texttt{hops}, $(pk_{j,
            V})_{j \in \{i, \dots, n\}}$, $(C_j)_{j \in \{i, \dots, n\}}$,
            \texttt{hops}[$i-1$]) to \adversary, expecting reply (\textsc{ok})
          \Else \: \Comment{\texttt{hops}[$i-1$] is corrupted}
            \State send (\textsc{hostDataBackward}, \texttt{hops}, $(pk_{j,
            V})_{j \in \{i, \dots, n\}}$, $(C_j)_{j \in \{i, \dots, n\}}$,
            \texttt{hops}[$i-1$]) to \adversary, expecting reply
            (\textsc{hostDataBackward}, \texttt{hops}, $(pk_{j, V})_{j \in
            \{k+1, \dots, n\}}$, $(C_j)_{j \in \{i, \dots, n\}}$,
            \texttt{hops}[$k$]) where \texttt{hops}[$k$] is the first honest hop
            before \texttt{hops}[$i-1$], or no response if there is no such hop
          \EndIf
        \EndIf
      \EndFor \: \Comment{control is at \texttt{host\_funder}}
      \State \Return ($(pk_{j, V})_{j \in [n]}$, $(C_j)_{j \in [n]}$)
    \end{algorithmic}
  \end{notitlebox}
  \caption{}
  \label{code:virtual-layer:host-init-data}
\end{figure}

\begin{figure}[H]
  \begin{notitlebox}{\textsc{virt.getTXs}($c$, $pk_{A, V}$, $pk_{B, V}$,
  $(pk_{j, V})_{j \in [n]}$, $(C_j)_{j \in [n]}$,
  $(\texttt{funding\_output}_j)_{j \in [n]}$)}
    \begin{algorithmic}[1]
      \For{$i$ in $\{1, n-1\}$} \Comment{new TXs for the $2$ hosts}
        \If{$C_i$ is a virtual layer TX}
          \State $T_{i, 1} \gets C_i$ with the following changes:
          \begin{itemize}
            \item the value of the ($\cdot$, $pk_{i, V} \wedge pk_{i+1, V}$)
            output is reduced by $c$
            \item output $(c, pk_{A, V} \wedge pk_{B, V})$ is added
            \item output $(0, \bigwedge\limits_{j \in [n]} pk_{j, V})$ is added
          \end{itemize}
        \Else \: \Comment{$C_i$ is the off-chain TX used in \textsc{PCN}}
          \State $T_{i, 1} \gets$ TX \{input: $\texttt{funding\_output}_i$,
          outputs: (($\texttt{funding\_output}_i\mathrm{.value} - c$,
          $\texttt{funding\_output}_i$.script), ($c$, $pk_{A, V} \wedge pk_{B,
          V}$), (0, $\bigwedge\limits_{j \in [n]} pk_{j, V}$))\} \TODO{needs
          \texttt{ANYPREVOUT}?}
        \EndIf
      \EndFor
      \For{$i$ in $2$ to $n-2$} \Comment{new TXs for intermediaries}
      \TODO{resolve this biggie: before the two sibling channels each had a
      separate commitment TX, now they share the same. It should be constructed
      on one side and shared with the other. What problems may this cause?}
        \State lala
      \EndFor
      \State \Return $(T_{i, j})_{i, j}$
    \end{algorithmic}
  \end{notitlebox}
  \caption{}
