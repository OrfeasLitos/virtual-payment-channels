  ...
  \begin{processbox}{\textsc{virt} -- close}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \State send (\textsc{read}) to \ledger as $R$ and assign reply to
        $\Sigma$
        \State $\texttt{keys} \gets \{(\pk{\mathrm{left}, \mathrm{guest}},
        \pk{\mathrm{right}, \mathrm{guest}})\}$
        \If{\texttt{funder} or \texttt{fundee} is defined}
          \State add $(\pk{\mathrm{loc}, \mathrm{virt}}, \pk{\mathrm{rem},
          \mathrm{virt}})$ to \texttt{keys}
        \EndIf
        \For{$(\pk{1}, \pk{2})$ in \texttt{keys}}
          \State ensure that $\Sigma$ has an unspent output that can be spent
          exclusively by a 2-of-\{$\pk{1}$, $\pk{2}$\} multisig \Comment{if
          there is a timelock, it must have expired}
        \EndFor
