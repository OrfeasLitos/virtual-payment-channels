  ...
  \begin{systembox}{$\mathcal{F}_{\mathrm{Chan}}$ -- init \& top up}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        0)$
        \State $\mathtt{virtuals} \gets \emptyset$
        \State $(\mathtt{funder}, \mathtt{fundee}, \mathtt{outer\_funder},
        \mathtt{outer\_fundee}) \gets (\bot, \bot, \bot, \bot)$
...
\begin{figure}[H]
  \begin{systembox}{$\mathcal{F}_{\mathrm{Chan}}$ -- close Pt. 1}
    \begin{algorithmic}[1]
      \TODO{handle arbitrarily nested virtuals (now we only handle one level and
      it leads to nested ifs}
      \State On (\textsc{close}) by \alice:
      \Indent
        \State ensure $\mathit{State} \in \{\textsc{open base}, \textsc{open
        virtual}\}$
        \State ensure \textsc{ln.close}() returns (\textsc{ok}) \TODO{continue}
