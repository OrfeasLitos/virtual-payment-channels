...
\begin{figure}[H]
  \begin{processbox}{\textsc{ln}.\textsc{submitAndCheckRevocation}($j$)}
    \begin{algorithmic}[1]
      ...
      \If{$\texttt{host}_P \neq \ledger$}
        \State send (\textsc{used revocation}) to $\texttt{host}_P$
      \EndIf
    \end{algorithmic}
  \end{processbox}
  \caption{}
  ...
  \begin{processbox}{\textsc{ln} -- On (\textsc{close}):}
    \begin{algorithmic}[1]
      ...
      \If{$\texttt{host}_P \neq \ledger$} \Comment{we have a virtual channel}
        ...
        receiving by $\texttt{host}_P$ output (\textsc{closed}) or
        (\textsc{guest punished})
        \If{output is (\textsc{guest punished})}
          \State $c_P \gets 0$ \Comment{the party that funded our channel
          avoided an attempted malicious closure and punised its counterparty,
          thus gained our coins. This is secure, as the funding party is local}
          \State give up token of execution
        \EndIf
        \State $\texttt{host}_P \gets \ledger$
      \EndIf
      \State $\itistate \gets \textsc{closing}$
