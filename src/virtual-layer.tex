\TODO{add polling for virtual layer}
\TODO{explicitly ensure that all hosts share the same sid -- only if we decide
that a functionality is needed after all}
\begin{figure}[H]
    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \Else \: \Comment{we are \texttt{host\_funder}}
          ...
          \mathrm{virt}}$, ($\pk{A, V}$, $\pk{B, V}$)) of $\bar{P}$ and assign
...
\begin{figure}[H]
  \begin{processbox}{\textsc{virt} -- close}
    \begin{algorithmic}[1]
      \State On input (\textsc{close}) by $P \in \{\texttt{guest},
      ...
      \Indent
        ...
        \If{$\texttt{host}_P \neq \ledger$} \Comment{\texttt{host} is a
        \textsc{virt}}
      ...
      \State On (\textsc{check chain for closing}) by $P \in \{\texttt{guest},
      \texttt{funder}, \texttt{fundee}\}$:
      \Indent
        \State ensure \textit{State} = \textsc{closing}
        \State send (\textsc{read}) to \ledger as $P$ and assign reply to
        $\Sigma$
        \If{$\texttt{tx} \in \Sigma$}
