    ...
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \If{$\mathrm{len}(\texttt{hops}) > 1$} \Comment{we are not the last hop}
          ...
          $\texttt{host}_{\bar{P}}'$, \texttt{host\_sibling}, $\pk{P, F}'$,
          $\pk{\bar{P}, F}'$, $\pk{\mathrm{sib}, \bar{P}, F}$, $c$) to
          $\texttt{host}_P'$ and expect reply (\textsc{host init ok})
        \Else \: \Comment{we are the last hop}
          \State input (\textsc{init}, $\texttt{host}_P$,
          $\texttt{host}_{\bar{P}}'$, $\pk{P, F}'$, $\pk{\bar{P}, F}'$, $c$) to
          new \textsc{virt} ITI $\texttt{host}_P'$ and expect reply
          (\textsc{host init ok})
        \EndIf
