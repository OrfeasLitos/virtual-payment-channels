\section{VLN Protocol}
  \label{protocol}
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{VLN}}$ (self is \alice{} always)}
      \begin{algorithmic}[1]
        ...
        \Function{GetKeys}{} \TODO{Remove if unneeded}
  ...
  \begin{figure}[H]
    ...
    \environment}
      \begin{algorithmic}[1]
        \State Upon receiving $\left(\textsc{openVirtualChannel}, \alice, \bob,
        \charlie, x, \mathit{tid}\right)$ from \environment:
        \Indent
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{commitmentSigned}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \mathtt{HTLCNo} - \mathtt{lastLocalSigned} + 1$}
          ...
          \For{$i$ from \texttt{lastLocalSigned} to \texttt{HTLCNo}}
            ...
            \mathtt{remoteDelay} \text{ relative}\right)\right)$\}
            \If{$\textsc{VerifyIBS}\left(\mathtt{HTLCSigs}_{n+1, i},
            \mathtt{localHTLC}_{n+1, i}, pt_{\mathrm{htlc}, n+1}\right) =
            \mathtt{false}$}
              \State close channel (as in Fig.~\ref{alg:protocol:close})
              \State \Return
            \EndIf
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ - \textsc{revokeAndAck}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \If{$\textsc{TestKey}\left(st_{\mathrm{com}, n}, pt_{\mathrm{com},
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \ForAll{channels}
            ...
            \textsc{revokeAndAck} AND the HTLC expires within $\left[t, t +
            \tochain{}\right]$}
