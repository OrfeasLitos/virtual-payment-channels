...
\begin{center}
  ...
  $\texttt{sig\_bal}_{\bar{P}}$, $C_{n-1, \mathrm{r}}$, $\mathrm{sig}_{n-1,
  \mathrm{r}, C}$, $\pk{n, R, i+2}$) by $\bar{P}$:}
    \begin{algorithmic}[1]
      ...
      \State $\itistate \gets \textsc{waiting to revoke virt comm}$
      \State input (\textsc{coop close}, $\texttt{sig\_bal}$, $C_{n-1,
      \mathrm{r}}$, $\mathrm{sig}_{n-1, \mathrm{r}, C}$, $\pk{n, R, i+2}$) to
      $\texttt{host}_P$
    \end{algorithmic}
  \end{processbox}
  \captionof{figure}{}
