\begin{figure}[H]
  ...
  \label{code:ln:init}
\end{figure}
\TODO{rethink and homogenize normal-foreign code variable names -- is A and B
ok?}
\begin{figure}[H]
  ...
  \begin{processbox}{\textsc{ln} -- open virtual}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \If{real world} \TODO{repair com txs according to openBase(), also using
        $pk_R$-- or simply dedup}
        ...
        \texttt{host\_funder}.\textsc{virt}
        \State \trustedcode{TODO: update to $C_{\mathrm{temp}}$ with hops and
        return (\textsc{ok})}
      \EndIndent
    \end{algorithmic}
  \end{processbox}
  ...
  \begin{processbox}{\textsc{ln} -- pay}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \State $C_{\mathrm{loc}, i+1} \gets C_{\mathrm{loc}, i}$ with
        $x$ coins moved from \alice's to \bob's output
        \State ensure \textsc{verify}($C_{\mathrm{loc}, i+1}$,
        $\mathrm{sig}_{\mathrm{payer}, C, i+1}$, $pk_{B, F}$) = True
        \State $R_{\mathrm{loc}, i+1} \gets$ TX \{input: $C_{\mathrm{rem},
        i+1}$.outputs.payee, output: ($c_B$, $pk_{A,
        \mathrm{out}}$)\}
        ...
        i+1}$.outputs.payer, output: ($c_A$, $pk_{B, \mathrm{out}}$)\}
        ...
        i+1}$) to \bob
        \State $R_{\mathrm{loc}, i+1} \gets$ TX \{input: $C_{\mathrm{rem},
        i+1}$.outputs.payer, output: ($c_A$, $pk_{B,
        \mathrm{out}}$)\}
        \State ensure \textsc{verify}($R_{\mathrm{loc}, i+1}$,
