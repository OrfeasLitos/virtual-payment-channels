\section{Efficiency Evaluation \& Simulations}
  ...
  range. Third, all random choices are drawn from the uniform distribution. Each
  party knows its next $10$, $100$, $10$ payments respectively in each workload.
  ...
  respectively. Average latency is high, as payments include slow on-chain
  payments and channel openings. Regarding fees, none of the $3$ protocols
  provide fee recommendations, so we used the same fees for all $3$ to avoid
  bias. These fees are not systematically chosen, therefore
  Fig.~\ref{graph:fees} provides relative, not absolute, fees.
