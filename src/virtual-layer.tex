\begin{figure}[H]
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \texttt{sibling}, $c$) by \texttt{guest}:
      \Indent
        \State store message contents and \texttt{guest}
        \Comment{\texttt{sibling} is missing for edge nodes}
      ...
      \Indent
        ...
        (\textsc{ok})
        \State output (\textsc{hosts ready}) to \texttt{funder}
      \EndIndent
      \Statex
...
\begin{figure}[H]
  ...
  \texttt{revoc\_by\_prev})}
    ...
    \If{len(\texttt{hops}[1:]) = 0} \Comment{we are \texttt{host\_fundee}}
