  ...
  \begin{processbox}{\textsc{virt}.\textsc{circulateKeysCoinsTimes}(\texttt{left\_data}):}
    \begin{algorithmic}[1]
      ...
      \texttt{host\_funder}}
        ...
        \mathrm{new}})_{j \in [i-1]}$, $(\pk{j, \mathrm{right}, \mathrm{fund},
        \mathrm{old}})_{j \in [i-1]}$, $(\pk{j, \mathrm{left}, \mathrm{fund},
        \mathrm{old}})_{j \in \{2, \dots, j-1\}}$, $(\pk{j, \mathrm{out}})_{j
        \in [i-1]}$, $(c_{j, \mathrm{left}})_{j \in [i-1]}$, $(c_{j,
        ...
        \texttt{host\_fundee}}
          ...
          $t_{i, \mathrm{left}}$) to \texttt{sibling}
          \State \trustedcode{store input as \texttt{left\_data} and parse it as
          ($(\pk{j, \mathrm{fund}, \mathrm{new}})_{j \in [i-1]}$, $(\pk{j,
          \mathrm{right}, \mathrm{fund}, \mathrm{old}})_{j \in [i-1]}$, $(\pk{j,
          \mathrm{left}, \mathrm{fund}, \mathrm{old}})_{j \in \{2, \dots,
          i\}}$, $(\pk{j, \mathrm{out}})_{j \in [i-1]}$, $(c_{j,
          \mathrm{left}})_{j \in [i]}$, $(c_{j, \mathrm{right}})_{j \in [i-1]}$,
          $(t_j)_{j \in [i-1]}$, $\sk{i, \mathrm{left}, \mathrm{fund},
          \mathrm{old}}$, $t_{i, \mathrm{left}}$}
  ...
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \Indent
        ...
        \wedge c_{\mathrm{loc}} \geq c_{\mathrm{virt}}$ \Comment{left parties fund
        virtual channel}
...
\begin{center}
  \begin{processbox}{\textsc{virt.siblingSigs}()}
    \begin{algorithmic}[1]
      ...
      \Statex
      \ForAll{$j \in [2, \dots, n-1] \setminus \{i\}$} \Comment{hop data encoded
