...
\subsection{Virtual Channels}
  ...
  \begin{figure}
    ...
    transactions. $\sigma$ is the signature}
  ...
  activations of non-negligent parties, $s = (2 + q)\texttt{windowSize}$,
  stemming from Proposition~\ref{prop:liveness}), or $p + \sum\limits_{j = 2}^{n
  - 1}(s - 1 + t_j)$ if the intermediary's channel is itself virtual, where
  $t_j$ is the required timelock of the $j$th base channel of the intermediary's
  channel. The only exception are virtual outputs that correspond to an interval
  that includes all parties, which can only be used as funding outputs for the
  virtual channel as its interval cannot be further extended, therefore the two
  separate spending methods and the associated timelock are dropped.
  ...
  needed to be added to Bitcoin. We show in Theorem~\ref{theorem:anyprevout}
  that variadic recursive virtual channels with $O(1)$ on-chain and
  subexponential off-chain transactions for each party cannot be constructed in
  Bitcoin without this flag. We hope this work provides additional motivation
  for this flag to be included in the future.
