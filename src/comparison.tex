...
% TODO: stress that we here compare the _variadic_ Elmo construction
  ...
  (calculated as $[\text{incoming messages} + \text{outgoing messages}]/2$, only
  for each party separately) and amount of data sent and stored. The data is
  counted as the sum of the relevant secret keys ($32$ bytes each), public keys
  ($33$ bytes each, compressed form), signatures ($71$ bytes each) and timelocks
  ($4$ bytes each). \TODO{add "auxiliary data", or specifically what other kind
  of data is counted, if needed} \TODO{see if any optimisations are used by
  other papers, decide if it makes sense to ignore them and change the next
  phrase} Optimisations are not considered.
