\begin{figure}[H]
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      \Indent
        \State ensure \textsc{virt.circulateVirtualKeys}() returns (\textsc{ok})
      ...
      \State \textsc{circulateVirtualKeys}(\texttt{left\_keys}):
      \Indent
        \If{\texttt{left\_keys} is given as argument} \Comment{we are not
        \texttt{host\_funder}}
          \If{we have a \texttt{sibling}} \Comment{we are not
          \texttt{host\_fundee}}
            \State input (\textsc{virtual keys forward}, (\texttt{left\_keys},
            $(\sk{\mathrm{loc}, \mathrm{virt}}, \pk{\mathrm{loc},
            \mathrm{virt}})$, ($\sk{\mathrm{loc}, F}$, $\pk{\mathrm{loc}, F}$),
            $\pk{\mathrm{rem}, F}$) to \texttt{sibling}
            \State \trustedcode{store input as \texttt{left\_keys}}
            \State \trustedcode{parse \texttt{left\_keys} as
            \texttt{far\_left\_keys}, $(\sk{\mathrm{loc}, \mathrm{virt}},
            ...
            \textsc{virt.circulateVirtualKeys}(\texttt{left\_keys}) of $\bar{P}$
            and assign returned value to \texttt{right\_keys}}
            \State \trustedcode{parse \texttt{right\_keys} as
            \texttt{far\_right\_keys}, $\pk{\mathrm{rem}, \mathrm{virt}}$}
            \State \trustedcode{output (\textsc{virtual keys back},
            \texttt{right\_keys}, ($\sk{\mathrm{loc}, F}$, $\pk{\mathrm{loc},
            F}$), $\pk{\mathrm{rem}, F}$)}
            \State store output as \texttt{right\_keys}
            \State parse \texttt{right\_keys} as \texttt{far\_right\_keys},
            ($\sk{\mathrm{sib}, F}$, $\pk{\mathrm{sib}, F}$), $\pk{\mathrm{sib},
            \mathrm{rem}, F}$
            \State \Return (\texttt{right\_keys}, $\pk{\mathrm{loc},
            \mathrm{virt}}$)
        ...
        \Else \: \Comment{we are \texttt{host\_funder}}
          \State call \textsc{virt.circulateVirtualKeys}($\pk{\mathrm{loc},
...
\begin{figure}[H]
  \begin{processbox}{\textsc{virt}}
    \begin{algorithmic}[1]
      ...
      $\pk{\mathrm{loc}, \mathrm{out}}$, $\{\pk{\mathrm{sec}, i}\}_{i \in 1
      \dots n}$, \texttt{comm\_keys\_loc}, \texttt{comm\_keys\_rem}, $P$):
      \Indent
  ...
  $\mathrm{sigs}_{\mathrm{byLeft}}$)}
    \begin{algorithmic}[1]
      ...
      are not \texttt{host\_funder}}
        \If{len(\texttt{hops}) $> 1$} \Comment{we are not \texttt{host\_fundee}}
